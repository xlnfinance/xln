...
renderRisk=(hist)=>{
  ...
  var hist = hist.reverse().slice(window.riskchart.data.datasets[0].data.length)

  for(h of hist){
    var date = Date.parse(h.date)
    var y = h.settled_delta/100

    window.riskchart.data.labels.push('hi')

    window.riskchart.data.datasets[0].data.push({
      x: date/1000,
      y: y
    })
