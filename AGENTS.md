...
- **Known-owner-decision gate:** Never ask the owner to reconfirm a decision already explicit in this file or the active task. In particular, H1 means the complete Rust process (transport, HTTP/operator surface, Runtime/Entity/Account, WAL, J watcher and J submission); TypeScript may supervise the process but may not execute H1 financial state transitions.
...
- **One production H1 under every driver:** Scenario, replay and HLT drivers may differ, but Rust H1 may not. HLT must launch the same production Rust binary, wire, state transitions, WAL and J integration used by normal scenarios; test-only financial execution or H1 mirrors are forbidden.
