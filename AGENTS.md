...
- **External model quorum:** Never wait on an exhausted provider quota when another approved auditor is available. Claude CLI uses only Opus 5 or Sonnet 5 with medium/high effort; never use Fable without a separate explicit owner request. Cursor CLI may use Grok 4.5 High, Grok Build, or Composer models. OpenCode may use any available model, preferring inexpensive independent reviewers. Auditors are code-only unless the owner asks otherwise; the primary agent independently verifies every finding against xln's bilateral model.
- **Audit waves:** Run at most two external/subagent auditors on one candidate. Verify and fix their concrete findings, prove the fixes at L1/L2, then give the updated bytes to the next pair so reviewers do not duplicate stale findings. Prefer two different model families in the final wave. Two clean independent verdicts on the stable candidate end code audit; do not keep adding reviewers unless a release gate exposes new code evidence.
- **Advisor sampling:** Keep the compact evidence-based ranking in `docs/audit/advisor-scorecard.md`. Sample higher-scoring Cursor, Grok, OpenCode and subagent reviewers more often, retain a 25% exploration share for untried/independent families, and independently verify every verdict. Owner quorum threshold is one verified reviewer; never block the owner on a protocol question when another approved reviewer can resolve it.
## 🔍 EXTERNAL AUDIT RULE
