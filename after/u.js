  ...
  async broadcast(method, args){
    ...
    switch(method){
      ...
      case 'propose':
        ...
        if(args[2]){
          var {stdout, stderr} = await asyncexec(`diff -urB . ../${args[2]}`)
          args[2] = stdout
        }
...
*/
  ...
  */
  ...
  async processBlock(block){
    ...
    const to_execute = await Proposal.findAll({where: {cron: K.usable_blocks}})
    l("Processing cron and delayed jobs "+to_execute.length)
    for(let job of to_execute){
      l(job)
      if(job.kindof == 'proposeCommand'){
...
listyo = () => {
  Proposal.findAll().then(ps=>ps.map(p=>{
    l(`${p.id} ${p.change} execute at ${p.cron}`)
  }))
}
