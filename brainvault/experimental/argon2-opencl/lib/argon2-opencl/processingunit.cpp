...
void ProcessingUnit::setPassword(std::size_t index, const void *pw,
                                 std::size_t pwSize)
{
    setPasswordWithParams(index, *params, pw, pwSize);
}

void ProcessingUnit::setPasswordWithParams(std::size_t index,
                                           const Argon2Params &jobParams,
                                           const void *pw,
                                           std::size_t pwSize)
{
    void *memory = runner.mapInputMemory(index);
    jobParams.fillFirstBlocks(memory, pw, pwSize,
...
void ProcessingUnit::clearMemory()
{
    runner.clearMemory();
}
