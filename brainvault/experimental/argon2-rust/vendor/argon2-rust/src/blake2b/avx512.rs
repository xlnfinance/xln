//! AVX2 + AVX-512VL compression for one BLAKE2b stream.
//!
//! BLAKE2b exposes four independent `G` functions per half-round, so a
//! 256-bit register is its natural vector width. Widening this state to ZMM
//! would leave half of every register unused. This backend keeps the AVX2
//! layout and its single-instruction shuffle rotations, but replaces the
//! three-instruction rotate-by-63 sequence with AVX-512VL's `vprorq`.
//!
//! The AVX-512 instruction is deliberately isolated in inline assembly. If
//! AVX-512F + AVX-512VL are enabled as LLVM function attributes, LLVM rewrites
//! the entire function to EVEX instructions; that version measured 3-7% slower
//! than AVX2 on Cloudflare's AMD EPYC. Keeping the function's generated code at
//! AVX2 and spelling only the one profitable EVEX instruction preserves the
//! faster schedule. Runtime dispatch still requires all three features before
//! this function pointer can be selected.
...
unsafe fn load4(src: *const u64) -> __m256i {
    ...
    // alignment requirement. AVX-512VL implies the AVX state used here.
...
unsafe fn set4(a: u64, b: u64, c: u64, d: u64) -> __m256i {
    // SAFETY: the caller established AVX2 + AVX-512F + AVX-512VL support.
...
unsafe fn g(
    ...
    // SAFETY: every intrinsic requires AVX2 and the inline `vprorq`
    // additionally requires the AVX-512F + AVX-512VL support established by
    // backend dispatch.
    unsafe {
        ...
        *b = _mm256_xor_si256(*b, *c);
        // SAFETY: the backend dispatcher requires AVX-512F + AVX-512VL before
        // exposing this function pointer. `vprorq ymm, ymm, imm8` operates
        // wholly in `b`, touches no memory or flags, and 63 is valid.
        asm!(
            "vprorq {value}, {value}, 63",
            value = inout(ymm_reg) *b,
...
unsafe fn diagonalize(b: &mut __m256i, c: &mut __m256i, d: &mut __m256i) {
    // SAFETY: the caller established AVX2 + AVX-512F + AVX-512VL; every lane
    // immediate is valid.
    unsafe {
...
unsafe fn round(
    ...
    let s = &SIGMA[r];
    // SAFETY: the caller established AVX2 + AVX-512F + AVX-512VL. The twelve
    // call sites keep `r` in range, and every SIGMA entry is less than 16.
    unsafe {
...
/// # Safety
///
/// The current CPU must support AVX2, AVX-512F and AVX-512VL.
// Only AVX2 is exposed to LLVM so it cannot rewrite the whole function to the
// slower EVEX schedule described in the module docs. The inline assembly in
// `g` contains the one opaque AVX-512VL instruction form; the safety contract
// and dispatch still require AVX-512F + AVX-512VL as well.
#[target_feature(enable = "avx2")]
pub(super) unsafe fn compress(
    ...
    // SAFETY: `#[target_feature]` establishes AVX2, and backend dispatch has
    // established AVX-512F + AVX-512VL for the inline `vprorq`. The fixed-size
    // arrays make every four-word load and store in bounds.
    unsafe {
