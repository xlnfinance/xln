...
test('locked native release builds are byte-reproducible', () => {
  ...
  try {
    ...
    const compileC = (output: string) => Bun.spawnSync({
      cmd: [
        'clang', '-std=c11', '-O3', '-flto', '-mcpu=apple-m1', '-pthread',
    ...
    expect(compileC(c1).exitCode).toBe(0);
    expect(compileC(c2).exitCode).toBe(0);
    normalize(c1);
    normalize(c2);
    ...
    for (const noWipe of [false, true]) {
      ...
      normalize(join(first, 'release/brainvault-argon2-rust'));
      normalize(join(second, 'release/brainvault-argon2-rust'));
      expect(sha256(join(first, 'release/brainvault-argon2-rust')))
        .toBe(sha256(join(second, 'release/brainvault-argon2-rust')));
    }
  } finally {
