/*
 * Original GPU layout: Ondrej Mosnacek's argon2-gpu.
 * The compact permutation below incorporates the equivalent arrangement from
 * hashcat's MIT-licensed Argon2 kernel by the Netherlands Forensic Institute.
 * See NOTICE and the per-file/upstream license notices in this directory.
 */
...
void swap_u64_private(__private ulong *left, __private ulong *right)
{
    const ulong value = *left;
    *left = *right;
    *right = value;
}

void transpose_permute_private(__private ulong values[4], uint thread)
{
    if (thread & 0x08) {
        swap_u64_private(&values[0], &values[2]);
        swap_u64_private(&values[1], &values[3]);
    }
    if (thread & 0x04) {
        swap_u64_private(&values[0], &values[1]);
        swap_u64_private(&values[2], &values[3]);
    }
}

uint shuffle_shift2_index(uint idx, uint thread)
{
    const uint delta = ((idx & 0x02) << 3) + (idx & 0x01);
...
void shuffle_block(struct block_th *block, uint thread,
                   __local struct u64_shuffle_buf *buf)
{
    ulong values[4] = { block->a, block->b, block->c, block->d };

    for (uint idx = 1; idx < 4; idx++) {
        values[idx] = u64_shuffle(values[idx], thread ^ (idx << 2), thread, buf);
    }
    transpose_permute_private(values, thread);
    for (uint idx = 1; idx < 4; idx++) {
        values[idx] = u64_shuffle(values[idx], thread ^ (idx << 2), thread, buf);
    }
    struct block_th round = { values[0], values[1], values[2], values[3] };
    g(&round);
    values[0] = round.a; values[1] = round.b;
    values[2] = round.c; values[3] = round.d;

    for (uint idx = 1; idx < 4; idx++) {
        values[idx] = u64_shuffle(values[idx],
            (thread & 0x1c) | ((thread + idx) & 0x03), thread, buf);
    }
    round = (struct block_th){ values[0], values[1], values[2], values[3] };
    g(&round);
    values[0] = round.a; values[1] = round.b;
    values[2] = round.c; values[3] = round.d;
    ...
    for (uint idx = 1; idx < 4; idx++) {
        values[idx] = u64_shuffle(values[idx],
            ((thread & 0x1c) | ((thread - idx) & 0x03)) ^ (idx << 2),
            thread, buf);
    }
    transpose_permute_private(values, thread);
    for (uint idx = 1; idx < 4; idx++) {
    ...
    for (uint idx = 1; idx < 4; idx++) {
        values[idx] = u64_shuffle(values[idx], shuffle_shift2_index(idx, thread), thread, buf);
    }
    round = (struct block_th){ values[0], values[1], values[2], values[3] };
    g(&round);
    values[0] = round.a; values[1] = round.b;
    ...
    for (uint idx = 1; idx < 4; idx++) {
        values[idx] = u64_shuffle(values[idx], shuffle_shift2_index(4 - idx, thread), thread, buf);
    }

    block->a = values[0]; block->b = values[1];
    block->c = values[2]; block->d = values[3];
}
