...
import { BIP39_ENGLISH } from '../src/core/bip39-english.ts';
import { resolveKdfParams } from '../src/core/primitives/kdf.ts';
import { copyAndWipe } from '../src/core/primitives/encoding.ts';
import { BRAINVAULT_V1, BRAINVAULT_V1_SPEC_ID, shardRequestFingerprint } from '../src/core/primitives/spec.ts';
import { acceptShard, createShardSlots, finalizeShards } from '../src/native/shard-collector.ts';
import {
  cliCreationCharacterError, cliDomainError, cliPasswordError, cliProgressStatusLine,
  publicErrorCode, publicErrorMessage,
} from '../src/cli/policy.ts';
import { deriveBrainVaultNative } from '../src/native/index.ts';
import {
...
const VECTOR_FILE = await Bun.file(`${PACKAGE_ROOT}/tests/data/vectors-v1.json`).json() as VectorFile;
...
test('promo banner stays inside an ultra-narrow terminal', () => {
  const script = [
    'set timeout 5',
    'spawn env COLUMNS=20 NO_COLOR=1 TERM=xterm-256color bun brainvault --promo',
    'expect "brainvault.sh"',
    'exec kill -INT [exp_pid]',
    'expect eof',
  ].join('\n');
  const result = Bun.spawnSync({
    cmd: ['expect', '-c', script], cwd: PACKAGE_ROOT, stderr: 'pipe', stdout: 'pipe',
  });
...
test('canonical manifest authenticates every listed source and native binary', async () => {
  const manifest = await Bun.file(`${PACKAGE_ROOT}/docs/manifest.sha256`).text();
  ...
  for (const required of [
    'docs/spec-v1.md',
    'tests/data/vectors-v1.json',
    'src/core/primitives/spec.ts',
    'src/core/primitives/kdf.ts',
    'src/core/index.ts',
    'src/native/hybrid.ts',
    ...
    'src/native/prebuilds/darwin-arm64/argon2.metallib',
  ...
  for (const entry of entries) {
    ...
    const bytes = new Uint8Array(await Bun.file(`${PACKAGE_ROOT}/${entry.path}`).arrayBuffer());
...
test('native worker crashes closed on a wrong spec handshake', async () => {
  const worker = new Worker(`${PACKAGE_ROOT}/src/native/workers/native.ts`);
...
test('browser worker rejects malformed KDF fields without returning a stack', async () => {
  const worker = new globalThis.Worker(new URL('../src/native/workers/browser.ts', import.meta.url));
  try {
...
test('native orchestration rejects worker crash, truncation, and unsafe concurrency', async () => {
  ...
  await expect(deriveBrainVaultNative(input, {
    workerPath: `${PACKAGE_ROOT}/tests/fixtures/worker-crash.ts`,
  })).rejects.toThrow('BRAINVAULT_TEST_WORKER_CRASH');
  await expect(deriveBrainVaultNative(input, {
    workerPath: `${PACKAGE_ROOT}/tests/fixtures/worker-truncated.ts`,
...
test('native worker reuse preserves output across worker counts', async () => {
  const workerPath = `${PACKAGE_ROOT}/tests/fixtures/worker-deterministic.ts`;
...
function runCliInputTty(
  ...
  const command = `${commandPrefix}${['bun', 'brainvault', ...extraArgs].join(' ')}`;
...
function runCliTty(extraArgs: readonly string[], confirmation = '') {
  const command = ['env', 'TERM=xterm-256color', 'COLUMNS=80', 'bun', 'brainvault', '--shards', '1', '--workers', '1', '--engine', 'native', ...extraArgs].join(' ');
  const script = [
    'set timeout 10',
...
function runSensitivePreflight(
  ...
  const script = [
    'set timeout 3',
    `set stty_init "rows ${rows} cols ${columns}"`,
    `spawn env -u COLUMNS -u LINES NO_COLOR=1 TERM=${term} bun brainvault ${modeArgs.join(' ')}`,
    'expect {',
...
function runAnimatedCliTty(noColor: boolean, columns = 80) {
  ...
  const script = [
    ...
    `spawn ${environment} bun brainvault --shards 100 --workers 32 --engine c-neon`,
    'expect "Username: "',
...
function runBrokenAcceleratorCliTty() {
  ...
  try {
    ...
    const localModules = join(PACKAGE_ROOT, 'node_modules');
    symlinkSync(existsSync(localModules) ? localModules : join(PACKAGE_ROOT, '..'), join(packageRoot, 'node_modules'));
    const executablePath = join(packageRoot, 'src/native/prebuilds/darwin-arm64/brainvault-argon2-metal');
    const failureFixture = join(PACKAGE_ROOT, 'tests/fixtures/native-failure.ts');
    ...
    const manifestPath = join(packageRoot, 'docs/manifest.sha256');
    const manifest = readFileSync(manifestPath, 'utf8').replace(
      /^[0-9a-f]{64}  src\/native\/prebuilds\/darwin-arm64\/brainvault-argon2-metal$/m,
      `${digest}  src/native/prebuilds/darwin-arm64/brainvault-argon2-metal`,
    );
    writeFileSync(manifestPath, manifest);

    const script = [
      'set timeout 30',
      `spawn env NO_COLOR=1 TERM=xterm-256color COLUMNS=80 bun ${join(packageRoot, 'brainvault')} --shards 1000 --workers 32`,
...
function runMissingNativeWorkerCliTty() {
  ...
  try {
    const packed = Bun.spawnSync({
      cmd: ['bun', 'pm', 'pack', '--ignore-scripts', '--destination', temp, '--quiet'],
      cwd: PACKAGE_ROOT, stderr: 'pipe', stdout: 'pipe',
    });
    if (packed.exitCode !== 0) throw new Error(`BRAINVAULT_TEST_PACK_FAILED:${packed.stderr.toString()}`);
    ...
    const localModules = join(PACKAGE_ROOT, 'node_modules');
    symlinkSync(existsSync(localModules) ? localModules : join(PACKAGE_ROOT, '..'), join(packageRoot, 'node_modules'));
    rmSync(join(packageRoot, 'src/native/workers/native.ts'));
    const script = [
      'set timeout 10',
      'spawn env TERM=xterm-256color COLUMNS=80 bun brainvault --shards 1 --workers 1 --engine native',
...
function runSignalDuringNativeDerivation(signal: 'TERM' | 'HUP') {
  ...
  try {
    ...
    const executablePaths = [
      join(packageRoot, 'src/native/prebuilds/darwin-arm64/brainvault-argon2'),
      join(packageRoot, 'src/native/prebuilds/darwin-arm64/brainvault-argon2-m3'),
    ];
    const fixturePath = join(PACKAGE_ROOT, 'tests/fixtures/native-hang.ts');
    ...
    writeFileSync(manifestPath, readFileSync(manifestPath, 'utf8')
      .replace(
        /^[0-9a-f]{64}  src\/native\/prebuilds\/darwin-arm64\/brainvault-argon2$/m,
        `${digest}  src/native/prebuilds/darwin-arm64/brainvault-argon2`,
      )
      .replace(
        /^[0-9a-f]{64}  src\/native\/prebuilds\/darwin-arm64\/brainvault-argon2-m3$/m,
...
function runAskCliTty() {
  const script = [
    'set timeout 10',
    'set stty_init "rows 24 cols 80"',
    'spawn env TERM=xterm-256color COLUMNS=80 bun brainvault --ask --engine native --workers 1 --multiplier 1',
    'expect "Username: "',
    'send "alice\\r"',
    'expect "Password (hidden; typing works): "',
    'send "secret123456\\r"',
...
test('hidden password cannot be recalled into a later visible prompt', () => {
  ...
  const passwordMode = runCliInputTty(['--password', '--engine', 'native'], [
    ...
    'expect "recovery work (up/down or j/k, Enter confirms)"',
    'expect "MAXIMUM  | 1,000,000 shards | about 100× work"',
    'send "\\033\\[A"', 'after 100', 'send "\\003"',
  ]);
...
test('--show-password covers repeat and password-manager confirmation explicitly', async () => {
  ...
  const managed = runCliInputTty([
    '--password', '--show-password', '--shards', '1', '--workers', '1', '--engine', 'native',
  ], [
    'expect "Username: "', 'send "alice\\r"',
...
test('password mode honors inline exact recovery work instead of prompting for a profile', async () => {
  const expectedSitePassword = await deriveSitePassword(VECTORS[0]!.expect.masterKey, 'example.com');
  const script = [
    'set timeout 10',
    'set stty_init "rows 24 cols 80"',
    'spawn env TERM=xterm-256color COLUMNS=80 bun brainvault --password --shards 1 --workers 1 --engine native',
    'expect "Username: "', 'send "alice\\r"',
    'expect "Password (hidden; typing works): "', 'send "secret123456\\r"',
    'expect {',
    '  "recovery work (up/down or j/k, Enter confirms)" { send "\\003"; expect eof; exit 42 }',
  ...
  expect(output).not.toContain('recovery work (up/down or j/k, Enter confirms)');
...
test('Ctrl+C exits the entire CLI and impossible RAM plans fail before allocation', () => {
  const script = [
    'set timeout 5',
    'spawn bun brainvault --shards 1 --workers 1',
    'expect "Username: "',
    'send "\\003"',
    'expect eof',
    'catch wait result',
  ...
  const aborted = Bun.spawnSync({ cmd: ['expect', '-c', script], cwd: PACKAGE_ROOT, stderr: 'pipe', stdout: 'pipe' });
  ...
  const passwordScript = [
    'set timeout 5',
    'set stty_init "rows 24 cols 80"',
    'spawn env TERM=xterm-256color COLUMNS=80 bun brainvault --password',
    'expect "Username: "',
    'send "alice\\r"',
    'expect "Password (hidden; typing works): "',
    'send "\\003"',
  ...
  const passwordAbort = Bun.spawnSync({
    cmd: ['expect', '-c', passwordScript], cwd: PACKAGE_ROOT, stderr: 'pipe', stdout: 'pipe',
  });
...
test('default wallet flow requires an explicit Standard, High, or Maximum work choice', () => {
  const selected = runCliInputTty([
    '--workers', '1', '--engine', 'native',
  ], [
    'expect "Username: "', 'send "profile-audit\\r"',
    'expect "Password (hidden; typing works): "', 'send "secret123456\\r"',
    'expect "recovery work (up/down or j/k, Enter confirms)"',
    'expect "STANDARD | 10,000 shards | recommended | 1× work"',
    'expect "HIGH     | 100,000 shards | about 10× work"',
    'expect "MAXIMUM  | 1,000,000 shards | about 100× work"',
    'send "\\r"',
    'expect "10,000 shards × 1 workers"',
    'expect "DERIVING"', 'send "\\003"',
  ]);
  expect(selected.exitCode).toBe(130);
  expect(selected.stdout.toString()).toContain('4 standard  ·  10,000 shards');
});
...
test('24-row sensitive output pages before the secondary wallet can displace the primary', () => {
  const script = [
    'set timeout 10',
    'set stty_init "rows 24 cols 80"',
    'spawn env TERM=xterm-256color COLUMNS=80 LINES=24 bun brainvault --shards 1 --workers 1 --engine native',
    'expect "Username: "', 'send "alice\\r"',
    'expect "Password (hidden; typing works): "', 'send "secret123456\\r"',
    'expect "Re-enter password to show recovery words (hidden; typing works), or press Enter to exit: "',
    'send "secret123456\\r"',
...
test('Ctrl+C inside sensitive view erases it and exits the entire CLI', () => {
  const command = ['env', 'TERM=xterm-256color', 'COLUMNS=80', 'bun', 'brainvault', '--shards', '1', '--workers', '1', '--engine', 'native'].join(' ');
  const script = [
    'set timeout 10',
    'set stty_init "rows 24 cols 80"',
    `spawn ${command}`,
...
test('native hang fixture emits the canonical progress protocol', () => {
  expect(readFileSync(`${PACKAGE_ROOT}/tests/fixtures/native-hang.ts`, 'utf8')).toContain("'BVP1 1\\n'");
});

test('TERM=dumb refuses recovery-word disclosure after a public derivation', () => {
  const script = [
    'set timeout 10',
    'spawn env TERM=dumb NO_COLOR=1 bun brainvault --shards 1 --workers 1 --engine native',
    'expect "Username: "', 'send "alice\\r"',
    'expect "Password (hidden; typing works): "', 'send "secret123456\\r"',
...
test('missing TERM refuses recovery-word disclosure after a public derivation', () => {
  const script = [
    'set timeout 10',
    'spawn env -u TERM NO_COLOR=1 bun brainvault --shards 1 --workers 1 --engine native',
    'expect "Username: "', 'send "alice\\r"',
    'expect "Password (hidden; typing works): "', 'send "secret123456\\r"',
    'expect {',
...
test('CLI benchmark honors inline advanced parameters', () => {
  const benchmark = Bun.spawnSync({
    cmd: [
      'bun', 'brainvault', '--bench', '--shards', '1', '--multiplier', '2',
      '--workers', '1', '--engine', 'native',
    ],
    cwd: PACKAGE_ROOT,
    stderr: 'pipe',
...
test('exact --shards remains distinct from legacy --factor in benchmark parsing', () => {
  const exact = Bun.spawnSync({
    cmd: ['bun', 'brainvault', '--bench', '--shards', '2', '--workers', '1', '--engine', 'native'],
    cwd: PACKAGE_ROOT,
    stderr: 'pipe',
    stdout: 'pipe',
  });
  ...
  const level = Bun.spawnSync({
    cmd: ['bun', 'brainvault', '--bench', '--level', '1', '--workers', '1', '--engine', 'native'],
    cwd: PACKAGE_ROOT,
    stderr: 'pipe',
    stdout: 'pipe',
  });
