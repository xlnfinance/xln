...
function normalize(path: string): void {
  ...
  const run = Bun.spawnSync({
    cmd: ['bun', `${PACKAGE_ROOT}/src/packaging/normalize-macho.ts`, path],
    stderr: 'pipe',
    stdout: 'pipe',
  });
...
test('shipped Apple Silicon artifacts target the M1-era macOS baseline', () => {
  for (const artifact of [
    ...
    const inspected = Bun.spawnSync({
      cmd: ['vtool', '-show-build', `${PACKAGE_ROOT}/src/native/prebuilds/darwin-arm64/${artifact}`],
  ...
  const metalStrings = Bun.spawnSync({
    cmd: ['strings', `${PACKAGE_ROOT}/src/native/prebuilds/darwin-arm64/argon2.metallib`],
    stderr: 'pipe', stdout: 'pipe',
  });
...
test('locked native release builds are byte-reproducible', () => {
  ...
  try {
    const cRoot = `${PACKAGE_ROOT}/src/native/source/c`;
    const sources = [
    ...
    expect(sha256(c1)).toBe(sha256(`${PACKAGE_ROOT}/src/native/prebuilds/darwin-arm64/brainvault-argon2`));
    ...
    for (const cpu of ['apple-m1', 'apple-m3'] as const) {
      for (const noWipe of [false, true]) {
        ...
        expect(sha256(firstBinary)).toBe(sha256(`${PACKAGE_ROOT}/src/native/prebuilds/darwin-arm64/${prebuildName}`));
      }
    }

    const metalRoot = `${PACKAGE_ROOT}/src/native/source/metal`;
    const metalSources = [
      'brainvault_argon2_metal.m',
      '../c/vendor/argon2/src/argon2.c',
      '../c/vendor/argon2/src/core.c',
      '../c/vendor/argon2/src/blake2/blake2b.c',
      '../c/vendor/argon2/src/thread.c',
      '../c/vendor/argon2/src/encoding.c',
      '../c/vendor/argon2/src/opt.c',
    ];
    const compileMetalHost = (output: string) => Bun.spawnSync({
      cmd: [
        'clang', '-std=c11', '-O3', '-flto', '-mcpu=apple-m1', '-mmacosx-version-min=11.0', '-fobjc-arc',
        '-D_DARWIN_C_SOURCE', '-D__SSE2__', '-I../c/compat',
        '-I../c/vendor/sse2neon', '-I../c/vendor/argon2/include',
        '-I../c/vendor/argon2/src', ...metalSources,
    ...
    expect(sha256(metalHost1)).toBe(sha256(`${PACKAGE_ROOT}/src/native/prebuilds/darwin-arm64/brainvault-argon2-metal`));
    ...
    expect(sha256(metalLib1)).toBe(sha256(`${PACKAGE_ROOT}/src/native/prebuilds/darwin-arm64/argon2.metallib`));

    const openclRoot = `${PACKAGE_ROOT}/src/native/source/opencl`;
    const openclSources = [
    ...
    expect(sha256(opencl1)).toBe(sha256(`${PACKAGE_ROOT}/src/native/prebuilds/darwin-arm64/brainvault-argon2-opencl`));
  } finally {
