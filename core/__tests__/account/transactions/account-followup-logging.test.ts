...
test('account committed followups use structured logging only', () => {
  const frameFollowups = readRuntimeSource('core/entity/tx/handlers/account/committed-frame-followups.ts');
  const htlcFollowups = readRuntimeSource('core/entity/tx/handlers/account/committed-htlc-followups.ts');
