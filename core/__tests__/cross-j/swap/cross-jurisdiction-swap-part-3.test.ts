...
describe('cross-jurisdiction hashledger swap', () => {
  ...
  test('duplicate fill notice is idempotent and a foreign route hash fails fast', async () => {
    ...
    expect(duplicate.newState.crossJurisdictionSwaps?.get(route.orderId)?.filledSourceAmount).toBe(500n);
  ...
  test('committed partial fill notice updates the source Hub route without an Account round trip', async () => {
    ...
    expect(
      hubResult.outputs.some(
        output => output.entityId === sourceUser && output.entityTxs?.some(tx => tx.type === 'accountInput'),
      ),
    ).toBe(false);

    const hubState = hubResult.workingReplica.state;
    expect(hubState.accounts.get(sourceUser)?.state.swapOffers.has(route.orderId)).toBe(true);
    const updatedRoute = hubState.crossJurisdictionSwaps?.get(route.orderId);
    ...
    expect(updatedRoute?.fillDenominator).toBe(65_535n);
  });
