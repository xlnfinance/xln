...
import { applyEntityHeightDurabilityBarrier } from '../../../../runtime/mempool/entity-height-barrier';
...
describe('Entity durable-height barrier', () => {
  ...
  test('startup batch: unmerged local command + loopback commit in one frame and lose no work', () => {
    ...
    // Plain inputs commit sequential heights inside one Runtime frame; each
    // commit is recorded per height in the WAL, so nothing waits.
    expect(applyEntityHeightDurabilityBarrier(env, runtimeInput, mempool, 11)).toBe(0);
    expect(runtimeInput.entityInputs).toEqual([hubConfig, loopback, followUp, emptyWake]);
    expect(mempool.entityInputs).toEqual([]);
  });

  test('keeps an empty consensus wake beside a local command', () => {
    ...
    expect(applyEntityHeightDurabilityBarrier(env, forward, forwardMempool, 11)).toBe(0);
    expect(forward.entityInputs).toEqual([hubConfig, emptyWake]);
    expect(forwardMempool.entityInputs).toEqual([]);
    ...
    expect(applyEntityHeightDurabilityBarrier(env, reverse, reverseMempool, 11)).toBe(0);
    expect(reverse.entityInputs).toEqual([emptyWake, hubConfig]);
    expect(reverseMempool.entityInputs).toEqual([]);
  });

  test('applies J-prefix and leader continuations together with a following command', () => {
    ...
    for (const continuation of continuations) {
      ...
      const mempool: RuntimeInput = { runtimeTxs: [], entityInputs: [] };
      expect(applyEntityHeightDurabilityBarrier(env, runtimeInput, mempool, 11)).toBe(0);
      expect(runtimeInput.entityInputs).toEqual([continuation, command]);
      expect(mempool.entityInputs).toEqual([]);
    }
  });
  ...
  test('keeps same-frame importReplica + two unmerged entity txs before the replica exists', () => {
    ...
    const mempool: RuntimeInput = { runtimeTxs: [], entityInputs: [] };
    expect(applyEntityHeightDurabilityBarrier(env, runtimeInput, mempool, 11)).toBe(0);
    expect(runtimeInput.entityInputs).toEqual([hubConfig, loopback]);
    expect(mempool.entityInputs).toEqual([]);
  });

  test('keeps one certificate height per lane while plain inputs from many sources pass', () => {
    const env = createEmptyEnv('entity-height-plain-fanin');
    const entityId = entity('ee');
    const signerId = signer('e1');
    installLane(env, entityId, signerId, 5);
    const remote = (byte: string): RoutedEntityInput => localCommand(entityId, signerId, byte, signer(byte));
    const plain = [remote('01'), remote('02'), remote('03'), remote('04')];
    const runtimeInput: RuntimeInput = { runtimeTxs: [], entityInputs: [...plain] };
    const mempool: RuntimeInput = { runtimeTxs: [], entityInputs: [] };
    expect(applyEntityHeightDurabilityBarrier(env, runtimeInput, mempool, 11)).toBe(0);
    expect(runtimeInput.entityInputs).toEqual(plain);

    const p6 = input(entityId, signerId, 6, 'c6');
    const p7 = input(entityId, signerId, 7, 'c7');
    const certified: RuntimeInput = { runtimeTxs: [], entityInputs: [p7, p6] };
    const certifiedMempool: RuntimeInput = { runtimeTxs: [], entityInputs: [] };
    expect(applyEntityHeightDurabilityBarrier(env, certified, certifiedMempool, 11)).toBe(1);
    expect(certified.entityInputs).toEqual([p6]);
    expect(certifiedMempool.entityInputs).toEqual([p7]);
  });
});
