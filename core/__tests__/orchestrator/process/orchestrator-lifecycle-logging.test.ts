...
test('managed custody children terminate when their spawning process is replaced', () => {
  const bootstrap = readFileSync(join(process.cwd(), 'core/orchestrator/bootstrap/custody-bootstrap.ts'), 'utf8');
  const daemon = readFileSync(join(process.cwd(), 'core/api/server/index.ts'), 'utf8');
...
test('managed child survives with its parent and exits after the exact parent is killed', async () => {
  const fixture = join(process.cwd(), 'core/__tests__/fixtures/process/parent-watch-process.ts');
