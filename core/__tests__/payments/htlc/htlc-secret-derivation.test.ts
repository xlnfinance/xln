...
test('proposer creates an unpredictable preimage outside consensus bytes', () => {
  ...
  try {
    ...
    expect(getRandomValues).toHaveBeenCalledTimes(generated.length);
    expect(getRandomValues.mock.calls.every(([bytes]) => bytes instanceof Uint8Array && bytes.byteLength === 32)).toBe(true);
  } finally {
