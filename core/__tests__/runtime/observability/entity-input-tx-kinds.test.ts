...
test('FRAME_LOG histogram splits accountInput kinds and nested certified payloads', () => {
  const ack = {
    type: 'accountInput',
  ...
  const counted = countEntityInputTxKinds([
    ...
    input(fromA.toUpperCase(), [certifiedAck]),
  ...
  expect(counted.txKinds).toEqual({
    ...
    'consensusOutput:accountInput:ack': 1,
    chat: 1,
  });
});
