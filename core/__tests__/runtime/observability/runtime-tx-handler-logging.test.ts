...
test('runtime tx import logs use structured logging and invalid j-height fails closed', () => {
  const txHandlers = readFileSync(join(process.cwd(), 'core/runtime/tx/tx-handlers.ts'), 'utf8');
  const jurisdictionImport = readFileSync(join(process.cwd(), 'core/runtime/j-submit/jurisdiction-import.ts'), 'utf8');
  const recovery = readFileSync(join(process.cwd(), 'core/runtime/recovery/restore-adapters.ts'), 'utf8');
