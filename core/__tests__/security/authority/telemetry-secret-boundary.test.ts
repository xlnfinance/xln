...
test('consensus logging has no raw payload escape hatch', () => {
  ...
  const sources = [
    ...
    'core/entity/consensus/entity-log.ts',
...
test('J adapters route failures through structured telemetry and never synthesize zero reads', () => {
  const repoRoot = join(import.meta.dir, '..', '..', '..', '..');
  const browserVm = readFileSync(join(repoRoot, 'core/jurisdiction/adapter/browservm/browservm-provider.ts'), 'utf8');
