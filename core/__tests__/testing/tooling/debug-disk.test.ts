...
const scriptPath = join(repoRoot, 'core/scripts/operations/diagnostics/debug-disk.ts');
...
describe('debug:disk', () => {
  ...
  test('package exposes the disk diagnostic script', () => {
    ...
    expect(packageJson.scripts?.['debug:disk']).toBe('bun core/scripts/operations/diagnostics/debug-disk.ts');
  });
});
