 ...
 */
import type { AccountFrame, AccountOutput, AccountReplica } from '../../../types/account';
...
import { assertNoUnilateralSettlementMutation, captureSettlementVector } from '../helpers';
...
export const commitAccountFrameTransition = async (
  ...
  const owner = timePerfPhase('account.commitTransition.beginOverlay', () =>
  ...
  try {
    for (const tx of frame.accountTxs) {
      ...
      const result = await timePerfPhase('account.commitTransition.applyTx', () =>
        applyAccountTx(
          draft,
          tx,
          frame.byLeft,
      ...
      if (!result.ok) {
        throw new Error(`Frame ${frame.height} commit failed: ${tx.type} - ${result.rejection.message}`);
      }
    ...
    const committed = timePerfPhase('account.commitTransition.commitOverlay', () => commitAccountTransition(owner));
    timePerfPhase('account.commitTransition.publish', () => publishAccountOverlay(account, committed.account));
