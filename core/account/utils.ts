...
import { DEFAULT_TOKENS, TRON_ONLY_DEFAULT_TOKENS, defaultTokensForJurisdiction } from '../jurisdiction/machine/config/default-tokens';
 ...
 * All core/business logic (rebalance, routing, UI, tests) must interpret
...
export function deriveDelta(delta: Delta, isLeft: boolean): DerivedDelta {
  ...
  return {
    ...
    ...perspective,
    totalCapacity,
    // Display-only bar; built on first read so hot capacity math never pays for it.
    get ascii(): string {
      return formatDeltaAscii(totalDelta, collateral, totalCapacity, effectiveOwnCreditWindow);
    },
  };
}
