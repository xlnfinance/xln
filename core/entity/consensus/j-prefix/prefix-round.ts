...
export const ensureLocalJPrefixAttestation = (
  ...
  const existing = pendingConfig ? undefined : replica.jPrefixRound;
  const sameRoundAlreadyVoted = Boolean(
    existing &&
    existing.targetEntityHeight === replica.state.height + 1 &&
    existing.baseHeight === replica.state.lastFinalizedJHeight &&
    existing.attestations.has(sourceValidatorId)
  );
  replica.jPrefixRound = mergeJPrefixAttestations(
    env,
    authorityReplica.state,
    sameRoundAlreadyVoted ? undefined : existing,
