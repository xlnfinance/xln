...
import { encodeAbiStaticWords } from '../../protocol/crypto/abi-static-words';
...
): string => keccakHexHash(encodeAbiStaticWords([
  KEY_DOMAIN,
  laneDomain(identity.lane),
  bytes32(identity.sourceEntityId, 'CONSUMPTION_SOURCE_ENTITY'),
  bytes32(identity.targetEntityId, 'CONSUMPTION_TARGET_ENTITY'),
])).toLowerCase();

const LANE_DOMAINS = new Map<string, string>();
const laneDomain = (lane: string): string => {
  let value = LANE_DOMAINS.get(lane);
  if (value === undefined) {
    value = domain(`xln.consumption-frontier.lane.${lane}.v1`);
    LANE_DOMAINS.set(lane, value);
  }
  return value;
};
...
// One frontier/node is hashed several times per applied output (proof
// inspection, the rebuilt path, the emitted proof). Hashes are pure functions
// of the parsed content, so a bounded content-keyed memo is exact.
const HASH_MEMO_MAX = 8_192;
const frontierHashMemo = new Map<string, string>();
const nodeHashMemo = new Map<string, string>();
const memoize = (memo: Map<string, string>, key: string, compute: () => string): string => {
...
const hashFrontier = (valueInput: ConsumptionFrontierValue): string => {
  ...
  const memoKey = [
    ...
    value.lastOutputHanko,
    quarantine?.conflictingSemanticHash ?? '',
    quarantine?.conflictingOutputHash ?? '',
    quarantine?.conflictingOutputHanko ?? '',
  ].join('|');
  return memoize(frontierHashMemo, memoKey, () => keccakHexHash(encodeAbiStaticWords([
    FRONTIER_DOMAIN,
    value.version,
    value.lastContiguousSeq,
    value.lastSemanticHash,
    value.count,
    value.lastOutputHash,
    keccakHexHash(value.lastOutputHanko),
    Boolean(quarantine),
    quarantine?.conflictingSemanticHash ?? ethers.ZeroHash,
    quarantine?.conflictingOutputHash ?? ethers.ZeroHash,
    quarantine ? keccakHexHash(quarantine.conflictingOutputHanko) : ethers.ZeroHash,
  ])).toLowerCase());
};
...
const hashParsedNode = (node: ConsumptionNode): string => {
  if (node.type === 'leaf') {
    const frontierHash = hashFrontier(node.value);
    return memoize(nodeHashMemo, `l|${node.key}|${frontierHash}`, () => keccakHexHash(
      encodeAbiStaticWords([LEAF_DOMAIN, 2, node.key, frontierHash]),
    ).toLowerCase());
  }
  return memoize(nodeHashMemo, `b|${node.bit}|${node.left}|${node.right}`, () => keccakHexHash(
    encodeAbiStaticWords([BRANCH_DOMAIN, 2, node.bit, node.left, node.right]),
  ).toLowerCase());
};
