...
export const terminalizeCrossJurisdictionRoutesOnFinality = (
  ...
  for (const { route, settledRatio } of settlements) {
    ...
    if (!routes) {
      throw haltRuntimeFailure(
        'CROSS_J_FINALITY_COLLECTION_MISSING',
        `CROSS_J_FINALITY_COLLECTION_MISSING:${route.orderId}`,
      );
    }
    const writable = getEntityCollectionValueForWrite(routes, route.orderId);
    if (!writable) {
      throw haltRuntimeFailure(
        'CROSS_J_FINALITY_ROUTE_FORK_MISSING',
        `CROSS_J_FINALITY_ROUTE_FORK_MISSING:${route.orderId}`,
      );
    }
    ...
    delete writable.pendingSourceRegistryReveal;
    delete writable.pendingTargetRegistryReveal;
    if (writable.status === 'intent' && !writable.sourcePull && !writable.targetPull) {
      transitionCrossJurisdictionRouteStatus(
        writable,
        'cancelled',
        Number(newState.timestamp || 0),
      );
      routes.set(writable.orderId, writable);
      addMessage(newState, `🌉 Cross-j route ${writable.orderId} cancelled before Pull lock on Account finality`);
      continue;
    }
    ...
    routes.set(writable.orderId, writable);
    addMessage(newState, `🌉 Cross-j route ${writable.orderId} terminal after dispute finality: ${terminal}`);
  }
};
...
const applyHashLadderRevealRegisteredJEvent = (context: FinalizedJEventContext): void => {
  ...
  for (const found of routes?.values?.() ?? []) {
    if (isCrossJurisdictionTerminalStatus(found.status)) continue;
    const rolePull = data.targetRole ? found.targetPull : found.sourcePull;
    const roleLeg = data.targetRole ? found.target : found.source;
    ...
    if (!route) {
      throw haltRuntimeFailure(
        'CROSS_J_REGISTRY_ROUTE_FORK_MISSING',
        `CROSS_J_REGISTRY_ROUTE_FORK_MISSING:${found.orderId}`,
      );
    }
    ...
    if (dirty) routes!.set(route.orderId, route);
  }
};
