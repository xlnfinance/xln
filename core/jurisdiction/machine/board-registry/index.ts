...
// Both keys are pure functions of their string inputs and are recomputed on
// every board lookup (several per hanko verification). Bounded memos.
const KEY_MEMO_MAX = 4096;
const stackKeyMemo = new Map<string, string>();
const entityKeyMemo = new Map<string, string>();
const memoize = (memo: Map<string, string>, key: string, compute: () => string): string => {
  const cached = memo.get(key);
  if (cached !== undefined) return cached;
  const value = compute();
  if (memo.size >= KEY_MEMO_MAX) memo.clear();
  memo.set(key, value);
  return value;
};
...
export const getCertifiedBoardStackKey = (
  ...
  stackKeyMemo,
  `${String(jurisdiction.chainId)}|${String(jurisdiction.depositoryAddress)}|${String(jurisdiction.entityProviderAddress)}`,
  () => getCertifiedBoardStackKeyUncached(jurisdiction),
);

const getCertifiedBoardStackKeyUncached = (
  jurisdiction: Pick<JurisdictionConfig, 'chainId' | 'depositoryAddress' | 'entityProviderAddress'>,
): string => {
...
export const getCertifiedBoardEntityKey = (stackKey: string, entityId: string): string => memoize(
  entityKeyMemo,
  `${String(stackKey)}|${String(entityId)}`,
