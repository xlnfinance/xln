...
const normalizeClaimEnvelope = (expectedJurisdictionRef: string, raw: JPrefixClaim): JPrefixClaim => {
  ...
  const blocks = raw.blocks.map(block => {
    const normalized = normalizeStrictJEventBlock(block, priorHeight, scannedThroughHeight, 'J_PREFIX');
...
export const hasCurrentRoundJPrefixAttestation = (replica: EntityReplica): boolean => {
  ...
  const attestation = round.attestations.get(normalizeText(replica.signerId));
  if (!attestation || !replica.jHistory) return false;
  // A signed prefix is immutable once the Entity frame commits. Until then,
  // watcher catch-up can still fill an already-scanned height. Re-attest the
  // exact current local claim instead of proposing a stale 1-block certificate
  // against a 2-block rebuild (J_PREFIX_LOCAL_PREFIX_MISMATCH).
  const local = localClaimAtHeight(
    replica.state,
    replica.jHistory,
    attestation.scannedThroughHeight,
  );
  const attested: JPrefixClaim = {
    jurisdictionRef: attestation.jurisdictionRef,
    baseHeight: attestation.baseHeight,
    scannedThroughHeight: attestation.scannedThroughHeight,
    tipBlockHash: attestation.tipBlockHash,
    eventHistoryRoot: attestation.eventHistoryRoot,
    rangeHash: attestation.rangeHash,
    blocks: attestation.blocks,
  };
  return claimKey(local) === claimKey(normalizeClaim(replica.state, attested));
};
...
const assertJPrefixCertificateMatchesLocalHistory = (
  ...
  if (claimKey(local) !== claimKey(verified.selected)) {
    throw new Error(
      `J_PREFIX_LOCAL_PREFIX_MISMATCH:${verified.selected.scannedThroughHeight}` +
      `:localRange=${local.rangeHash}:certRange=${verified.selected.rangeHash}` +
      `:localTip=${local.tipBlockHash}:certTip=${verified.selected.tipBlockHash}` +
      `:localBlocks=${local.blocks.length}:certBlocks=${verified.selected.blocks.length}` +
      `:localBase=${local.baseHeight}:certBase=${verified.selected.baseHeight}` +
      `:finalized=${state.lastFinalizedJHeight}` +
      `:histScan=${history?.scannedThroughHeight ?? 'none'}`,
    );
  }
  return verified;
};
