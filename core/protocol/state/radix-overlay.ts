...
class RadixOverlayTransaction<K, V> implements RadixOverlayOwner<K, V> {
  ...
  readonly #describedKeys = new Map<string | number, Readonly<{ id: string; key: K; keyBytes: Uint8Array }>>();
  ...
  describeKey(key: K): Readonly<{ id: string; key: K; keyBytes: Uint8Array }> {
    if (typeof key === 'number' || typeof key === 'string') {
      const cached = this.#describedKeys.get(key);
    ...
    if (typeof key === 'number' || typeof key === 'string') this.#describedKeys.set(key, described);
    return described;
  }
  ...
  put(key: K, value: V): void {
    ...
    this.#mutations.set(described.id, {
      ...
      // Own/hash at fold, not on every coalesced write. A 1000-tx swap frame
      // overwrites the same two delta keys repeatedly; sealing each put made
      // the overlay O(txs) copies of values the fold already owns once.
      value,
    });
  }
