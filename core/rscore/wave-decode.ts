...
import { accountTxWire, canonicalValueWire, type ShadowOutputRow } from './shadow-wire';
...
type WaveProposal = {
   ...
   * publisher sends a `ack_frame` rather than a `frame`.
   */
  bundledAck: Readonly<{
    height: number;
    frameHash: string;
...
type WaveSwapOffer = {
  ...
  crossJurisdiction: import('../types/cross-jurisdiction').CrossJurisdictionSwapRoute | null;
};
...
type WaveFrameVerdict =
  ...
  | {
      ...
      signedFrame: AccountFrame & { hanko: string };
    }
...
type WaveVerdict =
   ...
   * recursive AckFrame or a generic Failed child would create wire states the
   * authoritative transition never produced.
   */
  | { kind: 'ackFrameApplied'; ackVerdict: WaveAckVerdict; frameVerdict: WaveFrameVerdict }
  | { kind: 'ackFrameRejected'; phase: 'ack' | 'frame'; reason: string }
  | { kind: 'disputeApplied' }
  | { kind: 'disputeRejected'; reason: string }
  | { kind: 'boardHankoRefreshApplied'; events: string[] }
  | { kind: 'boardHankoRefreshRejected'; reason: string }
...
const decodeWavePayload = (value: unknown): Wave => {
  ...
  for (const [index, created] of createdAccounts.entries()) {
    const committedH1 = applied.some(row => {
      ...
      const verdict = row.verdict.kind === 'ackFrameApplied'
...
const decodeDisputeDraft = (value: unknown, field: string): WaveDisputeDraft | null => {
  if (value === null) return null;
  const row = rscoreWireTuple(value, 5, field);
  return {
    ...(row[0] === null
      ? {}
      : { hanko: `0x${Buffer.from(rscoreWireBytes(row[0], `${field}.hanko`)).toString('hex')}` }),
    hash: rscoreWireHex(row[1], `${field}.hash`, 32),
    proofBodyHash: rscoreWireHex(row[2], `${field}.proofBodyHash`, 32),
    nonce: rscoreWireInt(row[3], `${field}.nonce`),
    proposerIsLeft: rscoreWireBool(row[4], `${field}.proposerIsLeft`),
  };
};
...
const decodeFrameFields = (row: readonly unknown[], field: string): AccountFrame => ({
  ...
  stateHash: rscoreWireHex(row[6], `${field}.stateHash`, 32),
});

const decodeFrame = (value: unknown): AccountFrame & { hanko: string } => {
  const row = rscoreWireTuple(value, 8, 'frame');
  return {
    ...decodeFrameFields(row, 'frame'),
    hanko: `0x${Buffer.from(rscoreWireBytes(row[7], 'frame.hanko')).toString('hex')}`,
  };
};
...
const decodeCommittedFrame = (value: unknown): WaveCommittedFrame => {
  ...
  return {
    frame: decodeFrameFields(rscoreWireTuple(row[0], 7, 'committedFrame.frame'), 'committedFrame.frame'),
...
const decodeFrameVerdict = (value: unknown, field: string): WaveFrameVerdict => {
  ...
  switch (tag) {
    ...
    case 11: {
      const fields = rscoreWireTuple(row, 4, `${field}.disputeRequired`);
      return {
        kind: 'frameDisputeRequired',
        reason: rscoreWireText(fields[1], `${field}.reason`),
        evidenceSecrets: rscoreWireList(fields[2], `${field}.evidenceSecrets`)
          .map((entry, index) => {
            const evidence = rscoreWireTuple(entry, 2, `${field}.evidenceSecrets.${index}`);
            return {
              hashlock: rscoreWireText(evidence[0], `${field}.evidenceSecrets.${index}.hashlock`),
              secret: rscoreWireText(evidence[1], `${field}.evidenceSecrets.${index}.secret`),
            };
          }),
        signedFrame: decodeFrame(fields[3]),
      };
    }
    default:
...
const ACK_FRAME_PHASES = ['ack', 'frame'] as const;
...
const decodeVerdict = (value: unknown): WaveVerdict => {
  ...
  if ((tag >= 0 && tag <= 4) || tag === 11) return decodeFrameVerdict(row, 'verdict');
  ...
  switch (tag) {
    ...
    case 9: {
      const fields = rscoreWireTuple(row, 3, 'verdict.ackFrameApplied');
      return {
        kind: 'ackFrameApplied',
        ackVerdict: decodeAckVerdict(fields[1], 'verdict.ackFrameApplied.ackVerdict'),
        frameVerdict: decodeFrameVerdict(fields[2], 'verdict.ackFrameApplied.frameVerdict'),
      };
    }
    case 10: {
      const fields = rscoreWireTuple(row, 3, 'verdict.ackFrameRejected');
      const phaseValue = rscoreWireInt(fields[1], 'verdict.ackFrameRejected.phase');
      const phase = ACK_FRAME_PHASES[phaseValue];
      if (phase === undefined) {
        return rscoreWireDecodeFail(`verdict.ackFrameRejected.phase:${phaseValue}`);
      }
      return {
        kind: 'ackFrameRejected',
        phase,
        reason: rscoreWireText(fields[2], 'verdict.ackFrameRejected.reason'),
      };
    }
    case 12:
      rscoreWireTuple(row, 1, 'verdict.disputeApplied');
      return { kind: 'disputeApplied' };
    case 13: {
      const fields = rscoreWireTuple(row, 2, 'verdict.disputeRejected');
      return {
        kind: 'disputeRejected',
        reason: rscoreWireText(fields[1], 'verdict.disputeRejected.reason'),
      };
    }
    case 14: {
      const fields = rscoreWireTuple(row, 2, 'verdict.boardHankoRefreshApplied');
      return {
        kind: 'boardHankoRefreshApplied',
        events: rscoreWireList(fields[1], 'verdict.boardHankoRefreshApplied.events')
          .map((entry, index) => rscoreWireText(
            entry,
            `verdict.boardHankoRefreshApplied.events.${index}`,
          )),
      };
    }
    case 15: {
      const fields = rscoreWireTuple(row, 2, 'verdict.boardHankoRefreshRejected');
      return {
        kind: 'boardHankoRefreshRejected',
        reason: rscoreWireText(fields[1], 'verdict.boardHankoRefreshRejected.reason'),
      };
    }
    default:
...
const decodeOutput = (value: unknown): WaveOutput => {
  ...
  switch (rscoreWireInt(row[0], 'output.tag')) {
    ...
    case 3: {
      const fields = rscoreWireTuple(row, 19, 'output.swapOfferUpsert');
      ...
      return {
        ...
        offer: {
          ...
          crossJurisdiction: fields[18] === null
            ? null
            : decodeRscoreCanonicalValue(
                fields[18],
                'output.crossJurisdiction',
              ) as import('../types/cross-jurisdiction').CrossJurisdictionSwapRoute,
        },
      };
    }
    case 4: {
  ...
  switch (output.kind) {
    ...
    case 'swapOfferUpsert': {
      ...
      return [
        ...
        offer.quantizedGive,
        offer.quantizedWant,
        offer.crossJurisdiction === null ? null : canonicalValueWire(offer.crossJurisdiction),
      ];
    }
...
const disputeDraftWire = (draft: WaveDisputeDraft | null): RscoreWireValue =>
  draft === null
    ? null
    : [
        draft.hanko === undefined ? null : hexToBytes(draft.hanko, 'transcript.dispute.hanko'),
...
const frameVerdictWire = (verdict: WaveFrameVerdict): RscoreWireValue => {
  switch (verdict.kind) {
    ...
    case 'frameDisputeRequired':
      return [
        11,
        verdict.reason,
        verdict.evidenceSecrets.map((evidence) => [evidence.hashlock, evidence.secret]),
        frameWire(verdict.signedFrame),
      ];
    case 'frameRejected':
...
const verdictWire = (verdict: WaveVerdict): RscoreWireValue => {
  switch (verdict.kind) {
    ...
    case 'ackFrameApplied':
      return [
        9,
        ackVerdictWire(verdict.ackVerdict),
        frameVerdictWire(verdict.frameVerdict),
      ];
    case 'ackFrameRejected':
      return [10, verdict.phase === 'ack' ? 0 : 1, verdict.reason];
    case 'disputeApplied':
      return [12];
    case 'disputeRejected':
      return [13, verdict.reason];
    case 'boardHankoRefreshApplied':
      return [14, [...verdict.events]];
    case 'boardHankoRefreshRejected':
      return [15, verdict.reason];
    case 'failed':
