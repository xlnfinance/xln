...
import { findValueImportComponents, isValueModuleReference } from '../../operations/diagnostics/import-cycle-analysis';

const RUNTIME_ROOT = path.resolve('core');
const EXCLUDED_PATH = /\/(?:__tests__|qa|scenarios|scripts)\//;

const ROOT_ENTRYPOINTS = new Set(['core/runtime.ts']);
const MAX_VALUE_IMPORT_SCC_SIZE = 1;
...
const jurisdictionRootFiles = fs.readdirSync('core/jurisdiction', { withFileTypes: true })
...
const apiRootFiles = fs.readdirSync('core/api', { withFileTypes: true })
  .filter(entry => entry.isFile() && entry.name.endsWith('.ts'));
...
for (const { line, specifier } of runtimeTypeBarrelExports) {
  errors.push(`RUNTIME_TYPE_BARREL_EXPORT core/runtime/types.ts:${line}:${specifier}`);
}
