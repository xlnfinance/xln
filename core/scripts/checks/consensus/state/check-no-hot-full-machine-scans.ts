...
const OWNING_HOT_PATHS = [
  ...
  '/core/runtime/delivery/identity.ts',
  '/core/entity/tx/handlers/account/orderbook/same/results.ts',
];
...
const sourceFiles = program.getSourceFiles().filter(source => {
  ...
  return path.includes('/core/') && isOwningHotPath(path);
});

const ACCOUNT_WORK_INDEX_PATH = '/core/entity/consensus/account/work-index.ts';
