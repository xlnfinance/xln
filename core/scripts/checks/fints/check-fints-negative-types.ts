...
const requireBrandMintingGate = (): void => {
  const result = spawnSync(
    process.execPath,
    ['core/scripts/checks/fints/check-brand-minting.ts'],
    { cwd: ROOT, encoding: 'utf8' },
  );
  if (result.status !== 0) {
...
requireSuccess(
  ...
  'core/scripts/checks/fints/tsconfig.protocol-brands-positive.json',
);
requireFailure(
  'FINTS_PROTOCOL_BRANDS_NEGATIVE',
  'core/scripts/checks/fints/tsconfig.protocol-brands-negative.json',
  ['protocol-brands.negative.ts'],
);
requireSuccess(
  'FINTS_PROTOCOL_BRANDS_WIDENED',
  'core/scripts/checks/fints/tsconfig.protocol-brands-widened.json',
);
