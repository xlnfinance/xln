...
const SEED = requireEnv('CUSTODY_SEED');
...
const killStaleCustodyDaemon = async (): Promise<void> => {
  const pattern = `core/api/server/index.ts --port ${DAEMON_PORT} --host 127.0.0.1 --server-id custody-daemon-${DAEMON_PORT}`;
...
const startDaemon = async (): Promise<ManagedChild | null> => {
  ...
  const daemonChild = spawnBunChild(
    'custody-daemon',
    ['core/api/server/index.ts', '--port', String(DAEMON_PORT), '--host', '127.0.0.1', '--server-id', `custody-daemon-${DAEMON_PORT}`],
    {
      USE_ANVIL: 'true',
