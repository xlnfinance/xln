...
export const createDemoEntities = () => {
...
export const createChatTransactions = (author) => [
    { type: 'chat', data: { author, content: `Chat during trading!` } }
];
export const createBatchChatTransactions = (authors) => {
    const txs = [];
...
export const createTradingProposals = (proposer, message) => [
    { type: 'propose', data: {
            proposer,
            action: { type: 'collective_message', data: { message } }
        } }
];
export const createGovernanceProposals = (proposer, message) => [
    { type: 'propose', data: {
            proposer,
            action: { type: 'collective_message', data: { message } }
        } }
...
export const createCornerCaseTests = (entityIds) => {
    const tests = [];
...
export const createDemoServerInput = (entityInputs = []) => ({
    serverTxs: [],
    entityInputs
});
export const logEntitySummary = (replica, entityId) => {
    ...
    if (messages > 0) {
        ...
        if (messages > 10)
            console.log(`     ... and ${messages - 10} more`);
    }
    // Log proposals
    if (proposals > 0) {
        ...
        if (proposals > 5)
            console.log(`       ... and ${proposals - 5} more`);
    }
};
export const printEntitySummary = (env, entityId, config) => {
...
// === DEMO VERIFICATION ===
export const verifyDemoResults = (env, entityIds) => {
