...
// ── Ядро: заявки ──────────────────────────────────────────────────────────────
function newOrder(owner, clientId, side, priceTicks, qtyLots, tifGTCorIOC = 0, postOnly = false) {
    if (qtyLots <= 0) {
        emit({ kind: 'REJECT' });
        return;
    }
...
function fillAgainstLevel(levelIdx, takerSide, remaining, acc) {
    while (remaining > 0) {
        const head = levelHead[levelIdx];
        if (head === EMPTY)
            return remaining;
