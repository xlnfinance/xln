 ...
 */
function createR2RBatch(receivingEntity, tokenId, amount) {
    return {
...
export async function executeReserveToReserve(jurisdiction, fromEntity = PREFUNDED_ENTITY_1, toEntity = PREFUNDED_ENTITY_2, tokenId = 1, amount = '0.1' // In ether units as string
) {
    try {
...
if (import.meta.main) {
    ...
    executeReserveToReserve(TEST_JURISDICTION, PREFUNDED_ENTITY_1, PREFUNDED_ENTITY_2, 1, // ETH token
    '0.1' // 0.1 ETH
    ).then(result => {
        if (result.success) {
