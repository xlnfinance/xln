...
export class JEventWatcher {
    ...
    async processBlockRange(fromBlock, toBlock, env) {
        try {
            ...
            const [entityRegisteredEvents, controlSharesReleasedEvents, nameAssignedEvents, controlSharesReceivedEvents, reserveUpdatedEvents, reserveTransferredEvents, settlementProcessedEvents] = await Promise.all([
    ...
    async syncHistoricalEventsForNewEntities(env) {
        ...
        for (const entityId of newEntities) {
            ...
            try {
                ...
                const reserveEvents = await this.depositoryContract.queryFilter(this.depositoryContract.filters.ReserveUpdated(entityId), // Filter by entity
                0, // From block 0
                currentBlock);
