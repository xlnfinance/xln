...
export function createU32I32Map(initialCapacityPow2 = 1 << 20) {
    let cap = 1;
    ...
    function findSlot(k) {
        ...
        while (true) {
            const st = state[j];
            if (st === 0)
                return (firstTomb >= 0) ? firstTomb : j;
            if (st === 1 && keys[j] === k)
                return j;
            if (st === 2 && firstTomb < 0)
                firstTomb = j;
    ...
    return {
        ...
        has(id) { return this.get(id) !== -1; },
        set(id, idx) {
            id = id >>> 0;
            if (id === 0)
                throw new Error("orderId 0 is reserved");
            maybeGrow();
            const j = findSlot(id);
            if (state[j] === 1) {
                vals[j] = idx | 0;
        ...
        size() { return _size; },
        clear() { keys.fill(0); vals.fill(0); state.fill(0); _size = 0; _tombs = 0; },
    };
}
