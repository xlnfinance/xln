...
const initMsgpack = async () => {
    if (packr)
        return packr; // Already initialized
    try {
 ...
 */
export const encodeAsync = async (data) => {
    if (USE_MSGPACK) {
 ...
 */
export const decodeAsync = async (buffer) => {
    if (USE_MSGPACK) {
        const packrInstance = await initMsgpack();
        // Msgpack decoding with integrity verification
        const decodedTuple = packrInstance.unpack(buffer);
