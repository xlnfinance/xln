...
| file | role |
|------|------|
| `core/hanko/core.ts` | core primitives: sign, pack, unpack, recover, flashloan governance |
| `core/hanko/signing.ts` | consensus integration: signEntityHashes, buildQuorumHanko, verifyHankoForHash |
| `core/account/crypto.ts` | key derivation: BIP-39 + HMAC-SHA256, signDigest, verifyAccountSignature |
| `core/entity/consensus/input/consensus.ts` | entity proposal, precommit, timeout, and commit-input consensus |
| `core/entity/consensus/frame/application.ts` | deterministic frame apply and Hanko-bound output construction |
