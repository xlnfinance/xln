...
| Money math | 100 / 60 | `runtime/account/utils.ts`, `runtime/account/state/frame.ts`, `runtime/protocol/serialization/index.ts` | `deriveDelta()` is the source of truth for bilateral capacity/economics; frame delta integrity and BigInt serialization live here. |
...
| J-layer bridge | 90 / 75 | `runtime/jurisdiction/adapter/*`, `runtime/jurisdiction/machine/batch/index.ts`, `runtime/jurisdiction/machine/history/height.ts` | Chain adapters, batch construction, J-event ingestion, and J-height safety. |
