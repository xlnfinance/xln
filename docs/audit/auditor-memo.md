...
| Money math | 100 / 60 | `runtime/account/utils.ts`, `runtime/account/state/frame.ts`, `runtime/protocol/serialization.ts` | `deriveDelta()` is the source of truth for bilateral capacity/economics; frame delta integrity and BigInt serialization live here. |
