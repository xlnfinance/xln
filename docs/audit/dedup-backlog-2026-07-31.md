...
15. `runtime/entity/tx/handlers/account/index.ts` — split HTLC resolve, direct payment, and swap-side delta logic.
16. `runtime/entity/tx/handlers/account/index.ts` — centralize disputed-account gate instead of inline repeated checks.
17. `runtime/entity/tx/handlers/payments/settle.ts` — split workspace mutation, approval, execute, and auto-approve paths.
18. `runtime/entity/tx/handlers/payments/settle.ts` — unify post-settlement dispute-proof refresh logic.
19. `runtime/entity/tx/handlers/dispute/index.ts` — split disputeStart and disputeFinalize into separate files.
20. `runtime/entity/tx/handlers/dispute/index.ts` — move proof-body canonicalization helpers to shared proof module.
21. `runtime/entity/tx/handlers/dispute/index.ts` — centralize on-chain account read and stale-local-dispute cleanup.
22. `runtime/entity/tx/handlers/dispute/index.ts` — centralize current J-height lookup through one imported helper only.
...
29. `runtime/entity/scheduler/index.ts` — split periodic tasks and one-shot hooks into separate modules.
30. `runtime/entity/scheduler/index.ts` — extract dispute hook logic from rebalance and HTLC hook logic.
