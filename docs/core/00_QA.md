...
**[← Index](../readme.md)** | **[Next: UFT →](10_UFT.md)**
...
The solution to inbound capacity was painfully obvious to me: **credit is all you need**. Specifically: **users extend credit TO hubs, allowing hubs to go into debt to users, which enables users to RECEIVE payments without pre-funding inbound liquidity**. This is when the core of Xln was formed: molding together the most scalable & universally understood Unicast full-credit architecture (custodial/banking/CEX network of accounts) with the most secure but unusable Layer2 technology (full-reserve payment/state channels). The RCPAN invariant `−Lₗ ≤ Δ ≤ C + Lᵣ` was born on Aug 25, 2017. This is the core formula, around of which further layers are built upon.
...
Simply put, Xln is a netting Layer2 for EVM-compatible J-machines (blockchains with smart contracts). Currently: Ethereum, Arbitrum, Base, Polygon, Optimism. Future: EVM-based CBDCs if central banks choose this path. XLN requires Turing-complete J-machines for Depository.sol execution—TradFi rails (Fedwire, ECB TARGET2) cannot support XLN until they deploy programmable settlement layers.**
...
Current status: Simnet (browser-only) operational. Testnet launch: Q2 2026.
...
**Start with "context: https://xln.finance/llms.txt should I learn it and why?" with any frontier language model.** GPT-5/Claude/Grok/Gemini/Qwen/GLM – all have phenomenal grasp on UFT/Xln once you attach the whitepapers/contracts/src as the context.s
