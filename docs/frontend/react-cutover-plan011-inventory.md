...
This document records the implementation and stabilization checkpoint for the atomic React/Vite/TypeScript cutover. It is not production activation evidence. On 5 August 2026 the owner authorized all listed Runtime/Entity/Account work. The canonical React surfaces and authorized protocol fixes are implemented. The current handoff is frontend-only: preserved Runtime/MM work and its release gates are explicitly outside this checkpoint.
...
- Wallet remote-runtime boot now preserves the selected imported Runtime identity while the adapter hot-swap publishes its brief disconnected handle. A focused boot regression covers initial remote intent, pending H2, and connected H2 without reopening the vault screen.
- The canonical React Runtime selector replaces the retired dropdown-tree test contract. H1/H2/H3 are imported through the session-scoped capability registry, selection waits for both adapter and React state to converge, and remote Accounts exposes a read-only/local-signer-required boundary instead of calling the local-only faucet store.
- React Health parity retains the authoritative system verdict, source height/code hash, gates, hubs, process tree, storage, and explicit Runtime projection status. QA remains a separate link-only surface, and Health never owns or mutates Runtime credentials. Bare `/radapter` redirects to `/app`; query-bearing variants fail loudly with HTTP 400 in both Vite and generated Nginx routing.
- Market-maker bootstrap submits every bounded source-hub wave through one canonical plural command, producing one target-first and one source authorization input per wave without changing Account or Entity transition semantics. The 45-intent source-hub frame boundary remains unchanged; the experimental multi-hub frame was removed.
- Runtime storage retains the exact latest synced WAL frame hash in the process-local replica envelope, eliminating full prior-frame decoding after the first append without changing the WAL schema or hash chain. Reliable routed-output identities use a bounded process cache only for isolated split clones, with explicit invalidation on every merge mutation; canonical bytes and persisted values remain unchanged.
- The fresh production soundcheck is green in `.logs/bootstrap-soundcheck/2026-08-06T17-19-27-834Z`: all six cross routes reached required depth in 256,188 ms under the unchanged 300,000 ms budget, the complete system reached stable readiness in 270,174 ms, every pending-work counter drained to zero, and the canonical bootstrap hash remained `1489ea17f5a70952930b4cd8e4452a12d3a8067690d2b2abe2e8f39ba6ac1e89`.
- The final requested React batch passed 3/3 in 18.6 seconds with strict browser health in `.logs/e2e-parallel/20260806-172651-928`. The connected H1 overview, canonical H2 selector plus read-only Accounts boundary, and React Health screenshots were inspected; the browser logs contain zero unexpected console/page errors and only the two explicitly expected query-bearing `/radapter` HTTP 400 warnings.
- The CI browser batch exposed one remaining remote-admin spec bound to retired Svelte discovery, configure, profile-editor, and time-machine DOM. The reconciled flow preserves the user-level contract through canonical React Accounts and Activity: capability reload, exact-ID account review, a real add-asset `extendCredit` submission, observed receipt, exact WAL history frames, and filtered activity detail. The focused isolated scenario passes in 5.638 seconds of browser time in `.logs/e2e-parallel/20260806-175814-648` without raising its timeout or weakening Runtime assertions.
- The final frontend-only L1 batch passes 28/28 tests with 139 expectations. `bun run check` passes on the preserved worktree, including frozen-core hash `0x4eccf4492e5d085b24162f86d327e003c36b7e2a90ad527db1653fde391946a7` and all four React/Vite/TypeScript production builds.
- The exact connected React batch passes 3/3 with strict browser health in `.logs/e2e-parallel/20260806-191023-452`; its 1920×1080 H1/H2 selector and Health screenshots were inspected with no unexpected console/page failures. Separate 1440×900 laptop and iPhone 15 Pro frontend captures also had zero console errors or warnings.
- The dedicated connected desktop/mobile main-tabs capture remains blocked before Vite and Playwright: `.logs/e2e-parallel/20260806-191338-789` records `SHARD_BASELINE_RESET_TIMEOUT` while `reset_market_maker` owns the isolated prerequisite. Per the owner-corrected scope, the harness implementation was not rerun or changed. A seven-line, unimplemented timeout-helper expectation added to `runtime/__tests__/e2e-runner-isolation.test.ts` immediately before that correction remains preserved as an out-of-scope diff; no `gate:ci`, `gate:release`, commit, push, merge, tag, or activation is claimed by this handoff.
