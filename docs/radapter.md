...
The hub issues an expiring, audience-bound `xlnra1` capability for one Runtime.
The canonical operator link keeps the bearer secret in the URL fragment, which
is not sent to HTTP servers, proxies, or access logs:

```
https://xln.finance/app#runtime=remote&ws=wss%3A%2F%2Fhub.example.com%2Frpc&token=<admin-capability>
```

The frontend consumes and removes the fragment synchronously, then retains the
capability in `sessionStorage` only. Query-string Runtime credentials are
rejected; persistent browser storage never contains the bearer token.
