...
export interface RuntimeAdapterConfig {
  ...
  authKey?: string                // remote only — HMAC(seed, 'inspect'|'admin')
...
```
inspectKey = HMAC(runtimeSeed, 'inspect').slice(0, 32)
adminKey   = HMAC(runtimeSeed, 'admin').slice(0, 32)
```
...
- pick `/inspector/:entityId` (new route, no rewrite of existing UI)
