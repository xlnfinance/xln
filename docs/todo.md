...
- [ ] Split `EntityPanelTabs.svelte` into tab-owned components.
  ...
  - Progress: twenty-seventh extraction done. Pending-batch clear/broadcast/rebroadcast input construction and UI action runner moved into `pending-batch-actions.ts`; `EntityPanelTabs.svelte` is down to 3,793 lines and now delegates the batch workflow through a typed fail-fast controller.
  - Evidence: L1 `bun test tests/frontend/pending-batch-actions.test.ts tests/frontend/account-workspace-navigation.test.ts tests/frontend/entity-panel-routing.test.ts` PASS `12/12`; `bun run check:frontend-file-size` PASS; `bun run check:frontend` PASS with `svelte-check 0 errors / 0 warnings`; focused desktop/mobile main-tabs e2e `20260624-201044-119` PASS `1/1`, wall `31.9s`, HEAD `3ea7526fbd32 dirty`, code hash `a04082b535427762`, benchmark OK vs `20260624-194818-300`, browser issues `0`.
...
- [x] Enforce frontend source file-size invariant in the main check.
  ...
  - Evidence: `bun run check` PASS. Largest frontend files after the split are `SwapPanel.svelte` 3,798 lines, `EntityPanelTabs.svelte` 3,793 lines, `Graph3DPanel.svelte` 3,772 lines, `/qa/+page.svelte` 3,576 lines, and `vaultStore.ts` 3,567 lines.
...
- [x] Move Graph3D pure helpers out of the Svelte panel.
  ...
  - Progress: ninth extraction done. Renderer theme colors, WebGPU/WebGL renderer creation, and Object3D disposal moved into `graph3d-renderer.ts`; `Graph3DPanel.svelte` is down to 3,772 lines. WebGPU fallback no longer silently swallows initialization errors; default WebGL behavior remains unchanged.
  - Evidence: L1 `bun test tests/frontend/graph3d-helpers.test.ts tests/frontend/graph3d-settings.test.ts` PASS `13/13`; `bun run check:frontend-file-size` PASS; `bun run check:frontend` PASS with `svelte-check 0 errors / 0 warnings`; focused BrowserVM/Graph3D e2e `20260624-200238-134` PASS `1/1`, wall `11.3s`, HEAD `0684ff0b7034 dirty`, code hash `fb7cdd7e47e0b9e2`, benchmark OK vs `20260624-195745-520`, browser errors/network/http `0`, browser warnings `4` all WebGL `ReadPixels` performance warnings.
