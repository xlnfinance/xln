...
test('ALICE CREATES PROPOSAL → BOB VOTES → PROPOSAL EXECUTED + COLLECTIVE MESSAGE', async ({ page }) => {
  ...
  await secondPanel.getByRole('textbox', { name: 'Add a comment to your vote...' }).fill('I approve this budget allocation for Q4');
        ...
        for (const [id, prop] of proposals) {
          if (!proposalMap.has(id)) {
            proposalMap.set(id, {
              ...
              votes: prop.votes ? Array.from(prop.votes.entries()).map(([voter, voteData]): [string, string] => [
                voter,
