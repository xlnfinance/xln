...
test('CREATE ENTITY -> PROPOSAL -> VOTING WORKFLOW', async ({ page }) => {
  ...
  await firstPanel.getByRole('textbox', { name: 'Enter proposal description...' }).fill('Approve $100K budget for Q4 marketing campaign');
  ...
  await secondPanel.getByRole('textbox', { name: 'Add a comment to your vote...' }).fill('I approve this budget allocation');
