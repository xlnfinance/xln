  ...
  return (
    <div className="ops-graph-wrap" data-testid="scenario-graph" data-height={graph.height}>
      <svg viewBox="0 0 100 64" role="img" aria-label={`Runtime graph at height ${graph.height}`}>
        ...
        {graph.nodes.map(node => <g key={node.id} data-testid="scenario-node" transform={`translate(${node.x} ${node.y})`} className={`${node.hub ? 'is-hub' : ''} ${node.disputed ? 'is-disputed' : ''}`}><circle r={node.hub ? 4.1 : 3.2}/><text y="7" textAnchor="middle">{node.label}</text><title>{`${node.label}: ${node.accounts} accounts, ${node.debts} debts`}</title></g>)}
      </svg>
