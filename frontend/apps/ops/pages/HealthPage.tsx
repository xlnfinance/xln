    ...
    <section className="ops-page ops-health" data-testid="ops-health-page">
      ...
      {health ? <>
        <section className={`ops-verdict is-${health.verdict.toLowerCase()}`} data-testid="health-verdict-banner">
          ...
          <dl><div><dt>source frame</dt><dd>{health.sourceHeight ?? 'n/a'}</dd></div><div><dt>uptime</dt><dd>{uptime(health.uptimeMs)}</dd></div><div><dt>RPC</dt><dd>{state.rpc.ok === null ? 'unknown' : state.rpc.ok ? `${state.rpc.latencyMs}ms` : state.rpc.error}</dd></div><div><dt>code</dt><dd>{health.codeHash?.slice(0, 12) ?? 'n/a'}{health.dirty ? '-dirty' : ''}</dd></div></dl>
        </section>
        ...
        <section className="ops-metrics"><article><span>disk free</span><strong>{bytes(health.storage.freeBytes)}</strong></article><article><span>disk used</span><strong>{health.storage.usedPct ?? 'n/a'}%</strong></article><article><span>tracked stores</span><strong>{health.storage.tracked}</strong></article><article><span>Runtime entities</span><strong>{state.entities.length}</strong></article><article><span>activity window</span><strong>{state.events.length}</strong></article></section>
        {state.projectionError ? <p className="ops-notice" role="status">Runtime projection is explicitly unavailable: <code>{state.projectionError}</code></p> : null}
