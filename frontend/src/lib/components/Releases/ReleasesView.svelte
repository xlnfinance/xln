<script lang="ts">
  ...
  import { fetchReleaseDocument, fetchVerifiedReleaseManifest, RELEASE_METRICS, type ReleaseManifest, type ReleaseMetricKey } from '$lib/releases/release-catalog';
  import { deriveReleaseChartPath, deriveReleaseChartPoints, formatReleaseMetric, getReleaseMetricValue, getReleaseScopes } from '$lib/releases/release-chart';
  import { verifyReleaseManifestEntry } from '$lib/releases/release-signature';

  let manifest = $state<ReleaseManifest | null>(null);
  let selectedVersion = $state('');
  let selectedMetric = $state<ReleaseMetricKey>('code');
  ...
  let scopes = $derived(getReleaseScopes(manifest?.releases ?? []));
  let chartPoints = $derived(deriveReleaseChartPoints(manifest?.releases ?? [], selectedScope, selectedMetric));
  let chartPath = $derived(deriveReleaseChartPath(chartPoints));

  function formatMetric(value: number): string {
    return formatReleaseMetric(value, selectedMetric);
  }
  ...
  async function loadRelease(version: string): Promise<void> {
    ...
    renderedMarkdown = await fetchReleaseDocument(release);
  }
  ...
  onMount(async () => {
    try {
      manifest = await fetchVerifiedReleaseManifest();
      ...
      <div class="metric-controls">
        <label>
          ...
          <select bind:value={selectedMetric}>
            {#each RELEASE_METRICS as metric}
        ...
        <div class="current-value">
          <span>Current</span>
          <strong>{formatMetric(getReleaseMetricValue(selectedRelease, selectedScope, selectedMetric))}</strong>
        </div>
      </div>
