 ...
 *
 * Counting any transaction marks nearly every frame as interesting, because consensus
 * bookkeeping is transactions too. A story beat is an activity event that moved value or
 * changed account structure — everything else is classified `system`.
 */
const snapshotIsStoryBeat = (snapshot: EnvSnapshot): boolean =>
  ...
  return {
    ...
    readIndex: async () => normalizeRuntimeTimelineIndex({
      ...
      frames: Array.from(byHeight.values()).map((snapshot) => ({
        ...
        stateHash: String((snapshot as EnvSnapshot & { stateHash?: string }).stateHash || ''),
        materialized: true,
        graphChanged: snapshotIsStoryBeat(snapshot),
      })),
    }),
