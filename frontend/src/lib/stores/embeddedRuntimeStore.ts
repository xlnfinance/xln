...
const bootstrapEnvironment = writable<RuntimeState | null>(null);

// Active env is derived from selected runtime in dropdown. A selected remote
// runtime intentionally has no RuntimeState; remote UI reads RuntimeView projections.
...
let localDebugEnv: RuntimeState | null = null;
registerDebugSurface('env', () => localDebugEnv);

export function setXlnEnvironment(env: RuntimeState | null): void {
  ...
  if (canPublishActiveEnv) {
    ...
    errorLog.log(message, 'Runtime RuntimeState', { envRuntimeId: envRuntimeId || null, selectedRuntimeId });
