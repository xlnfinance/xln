  ...
  describe("processBatchWithHanko", function () {
    it("processes a reserve transfer using mock Hanko approval", async function () {
      ...
      const BatchType =
        ...
        "tuple(bytes32 counterentity,tuple(int256[] offdeltas,uint256[] tokenIds,tuple(address subcontractProviderAddress,bytes encodedBatch,tuple(uint256 deltaIndex,uint256 rightAllowance,uint256 leftAllowance)[])[] subcontracts) proofbody,bytes initialArguments,bytes finalArguments,bytes sig)[]," + // cooperativeDisputeProof
