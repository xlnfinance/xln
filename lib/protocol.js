// Not used. Will parse DNS. From https://github.com/trevoro/node-named/blob/master/lib/protocol.js
...
function parseIPv4 (addr) {
  if (typeof (addr) !== 'string') { throw new TypeError('addr (string) is required') }

  var octets = addr.split(/\./).map(function (octet) {
    return (parseInt(octet, 10))
  })
  if (octets.length !== 4) { throw new TypeError('valid IP address required') }
...
var serializers = {
  ...
  '_nsName': {
    encoder: function (v) {
      if (typeof (v) !== 'string') { throw new TypeError('name (string) is required') }
      var n = v.split(/\./)
