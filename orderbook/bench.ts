...
const flag = (k: string) =>
  argv.some(x => x === `--${k}` || x === `--${k}=1` || x === `--${k}=true`);

const MODE       = ((): "throughput" | "fidelity" => {
...
const pick   = <T>(a:T[]) => a[(rndU32()%a.length)|0];

/* ================================
   Throughput-mode pregen (synthetic)
   ================================ */

type K = 0 | 1 | 2; // 0=ADD, 1=CANCEL, 2=REPLACE
...
let   liveWr   = 0;
const livePush = (id:number)=>{ liveIds[liveWr&(LIVE_RING_SIZE-1)]=id; liveWr++; };
const livePick = ()=> {
  if (liveWr===0) return 1;
...
function pregenThroughput(){
  ...
  const hotAsk:number[]=[]; const hotBid:number[]=[];
  ...
  for (let i=0;i<OPS;i++){
    ...
    if (r<ADD_PCT){
      kinds[i]=0;
      const s=(rndU32()&1) as Side; sides[i]=s;
      const doCross   = rnd01()<CROSS_PCT;
      const doCluster = rnd01()<CLUSTER_PCT;
      let levelIdx:number;
      if (doCross) levelIdx = (s===0)? pick(hotAsk): pick(hotBid);
      else if (doCluster) levelIdx = (s===0)? pick(hotBid): pick(hotAsk);
      else {
        const offs=((rndU32()%(BAND_FAR*2+1))-BAND_FAR)|0;
        levelIdx=Math.max(0, Math.min(LEVELS-1, mid+offs));
      }
      prices[i]=pmin+levelIdx*tick;
      qtys[i]=((rndU32()%3|0)+1) as number;
      const id=nextId++; orderIds[i]=id; livePush(id);

    } else if (r<ADD_PCT+CANCEL_PCT){
      kinds[i]=1; orderIds[i]=livePick();

    } else {
      kinds[i]=2; orderIds[i]=livePick();
      if (rnd01()<REPRICE_PCT){
        reprice[i]=1; const s=(rndU32()&1) as Side; sides[i]=s;
        const li=(s===0)? pick(hotAsk): pick(hotBid);
        prices[i]=pmin+li*tick; deltas[i]=((rndU32()%3|0)-1)|0;
      } else {
        prices[i]=0; deltas[i]= -(((rndU32()%3|0)+1)|0);
      }
    }
  }
}
...
const liveBuy: number[] = [];   // orderIds of live bids
const liveSell: number[] = [];  // orderIds of live asks

/** Guarded push/pop for live pools */
function poolPush(side: Side, id: number){
  (side===0? liveBuy: liveSell).push(id);
}
function poolPick(side?: Side): number | null {
  const pool = (side===undefined) ? ((rndU32()&1)? liveBuy: liveSell) : (side===0? liveBuy: liveSell);
  if (!pool.length) return null;
  const i = rndU32()%pool.length|0;
  return pool[i] ?? null;
}
function poolRemove(side: Side, id: number){
  const pool = side===0? liveBuy: liveSell;
  const i = pool.indexOf(id);
  if (i>=0){ pool[i]=pool[pool.length-1]; pool.pop(); }
}
...
let nextExtId = 1;
function placeMaker(side: Side, near: boolean){
...
function placeTaker(){
  ...
  if (estBestBid<0 || estBestAsk<0) {
    ...
    placeMaker((rndU32()&1) as Side, true);
    return;
  }
...
function doCancel(){
  ...
  const pickSide: Side | undefined = (rndU32()%2) ? ((rndU32()%2) as Side) : undefined;
...
const loadSnapDb = (): Record<string, string> => {
  try { if (fs.existsSync(SNAP_DB)) return JSON.parse(fs.readFileSync(SNAP_DB,"utf8")); } catch {}
  return {};
};
const saveSnapDb = (db: Record<string, string>) => fs.writeFileSync(SNAP_DB, JSON.stringify(db, null, 2), "utf8");
...
function fmtEvent(e: ReturnType<typeof drainEvents>["items"][number]): string {
  switch (e.k) {
...
function runAscii(){
  ...
  for (let i=0;i<OPS;i++){
    if (MODE==="throughput"){
      ...
      if (k===0){ const id=orderIds[i], s=sides[i] as Side, px=prices[i], q=qtys[i]; console.log(`\n${BOLD}#${i+1} ADD${RESET} owner=${id} side=${s===0?GREEN+'BUY'+RESET:RED+'SELL'+RESET} px=${px} qty=${q}`); newOrder(id,id,s,px,q,0,false,false); }
      else if (k===1){ const id=orderIds[i]; console.log(`\n${BOLD}#${i+1} CANCEL${RESET} owner=${id} id=${id}`); cancel(id,id); }
      else { const id=orderIds[i]; if (reprice[i]){ const np=prices[i], d=deltas[i]|0; console.log(`\n${BOLD}#${i+1} REPLACE${RESET} owner=${id} id=${id} newPx=${np} dQty=${d}`); replace(id,id,np,d); } else { const d=deltas[i]|0; console.log(`\n${BOLD}#${i+1} REPLACE-REDUCE${RESET} owner=${id} id=${id} delta=${d}`); replace(id,id,null,d); } }
    } else {
    ...
    for (const e of drained.items){
      ...
      if (e.k==="CANCELED"){
        poolRemove(0, e.id); poolRemove(1, e.id);
      }
  ...
  for (let i=0;i<OPS;i++){
    if (MODE==="throughput"){
      ...
      if (!USE_API){
        ...
        else { if (reprice[i]) replace(orderIds[i],orderIds[i],prices[i],deltas[i]|0); else replace(orderIds[i],orderIds[i],null,deltas[i]|0); replaces++; }
      } else {
        let cmd: Cmd;
      ...
      if (wantCancel){ doCancel(); cancels++; }
      else if (wantReplace){ doReplace(); replaces++; }
      else {
        const depthWeak = (estBestBid<0 || estBestAsk<0) || (estBestAsk - estBestBid <= params.tick);
        const restingWeak = (liveBuy.length + liveSell.length) < MIN_RESTING;
        const doTaker = !depthWeak && !restingWeak && (rnd01() < TAKER_RATE);

        if (doTaker){ placeTaker(); }
        else { placeMaker((rndU32()&1) as Side, rnd01()<MAKER_BIAS_NEAR); }
        adds++;
      }
    }
    ...
    for (const e of drained.items) {
      ...
      if (e.k==="TRADE") {
        ...
        if (e.remain===0){ poolRemove(0, e.id); poolRemove(1, e.id); }
    ...
    if (LOG_EVERY && ((i+1)%LOG_EVERY===0)){
      process.stdout.write(
        `tick ${String(i+1).padStart(8)} | rest=${String(liveBuy.length+liveSell.length).padStart(6)} | workProof=${workProof.toString()}      \r`
      );
      if (ASCII_TICK) {
