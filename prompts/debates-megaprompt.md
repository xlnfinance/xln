...
8. `core/api/runtime-adapter/security/auth.ts`
9. `core/jurisdiction/adapter/default-tokens.ts`
10. `ai/server.ts`
...
- `core/api/runtime-adapter/security/auth.ts`: capability-token auth between service and runtime daemon.
- `core/jurisdiction/adapter/default-tokens.ts`: dev/test token catalog: USDC, WETH, USDT.
