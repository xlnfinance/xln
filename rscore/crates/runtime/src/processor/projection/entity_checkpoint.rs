...
pub(crate) struct PreparedEntityCheckpoint {
    ...
    pub(crate) metadata: EntityCheckpointProjectionMetadata,
}
...
pub(crate) fn prepare_entity_checkpoint(
    ...
    let retained = replica
        ...
        .as_ref()
        .ok_or(EntityCheckpointProjectionError::MetadataMissing)?;
    if retained.entity_id() != &owner {
        return Err(EntityCheckpointProjectionError::MetadataOwner);
    }
    let (account_owner, protocol_fingerprint) = retained
        .account_authority()
        .ok_or(EntityCheckpointProjectionError::AccountAuthorityMissing)?;
    ...
    let mut metadata = EntityCheckpointProjectionMetadata::new(
        owner,
        fields.values().cloned().collect(),
        trees.values().cloned().collect(),
    );
    if !metadata.bind_account_authority(account_owner, protocol_fingerprint) {
        return Err(EntityCheckpointProjectionError::MetadataOwner);
    }
...
fn manifest_bytes(
    ...
    let bytes = crate::transport::msgpack::encode_framed(&object([
        ("schemaVersion", Value::Number(2_u64.into())),
...
pub(crate) enum EntityCheckpointProjectionError {
    #[error("RRS_CHECKPOINT_ENTITY_METADATA_MISSING")]
    MetadataMissing,
    #[error("RRS_CHECKPOINT_ENTITY_METADATA_OWNER")]
    MetadataOwner,
    #[error("RRS_CHECKPOINT_ENTITY_ACCOUNT_AUTHORITY_MISSING")]
    AccountAuthorityMissing,
