//! The v1 Runtime frame header: one fixed-width block header per frame.
//!
...
//! of the whole header. Detailed parity diagnostics remain derived checks and
//! are not separate Runtime-frame identities.
//!
...
const RHEADER_DOMAIN: &[u8] = b"xln.runtime.header:v1";

pub const RHEADER_VERSION: u16 = 1;
/// version + height + timestamp + runtime_id + prev + input + state + outbox.
pub const RHEADER_BYTES: usize = 2 + 8 + 8 + 20 + 32 + 32 + 32 + 32;
...
impl RuntimeHeader {
    pub fn encode(&self) -> [u8; RHEADER_BYTES] {
        ...
        bytes[18..38].copy_from_slice(&self.runtime_id);
        bytes[38..70].copy_from_slice(&self.prev_r_hash);
        bytes[70..102].copy_from_slice(&self.r_input_root);
        bytes[102..134].copy_from_slice(&self.r_state_root);
        bytes[134..166].copy_from_slice(&self.r_outbox_root);
        bytes
    }
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, RuntimeHeaderError> {
        ...
        let field =
            |range: std::ops::Range<usize>| -> [u8; 32] { bytes[range].try_into().expect("width") };
        Ok(Self {
            version,
            height: u64::from_be_bytes(bytes[2..10].try_into().expect("width")),
            timestamp: u64::from_be_bytes(bytes[10..18].try_into().expect("width")),
            runtime_id: bytes[18..38].try_into().expect("width"),
            prev_r_hash: field(38..70),
            r_input_root: field(70..102),
            r_state_root: field(102..134),
            r_outbox_root: field(134..166),
        })
    }
...
{
    let mut hasher = Sha256::new();
    hasher.update(b"xln.runtime.outbox:v1");
    for row in rows {
...
pub fn state_root<'a>(
    ...
    let mut hasher = Sha256::new();
    hasher.update(b"xln.runtime.state:v1");
