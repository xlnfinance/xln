...
describe('numbered Entity registration authority', () => {
  ...
  test('batch uses the exact trusted RuntimeReplica adapter and explicitly selected vault payer', async () => {
  ...
  test('uses the exact trusted RuntimeReplica adapter and explicitly selected vault signer', async () => {
    const chainId = 31_338;
    const adapter = await createJAdapter({ mode: 'browservm', chainId });
    try {
