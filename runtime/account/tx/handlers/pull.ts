...
type CrossPullRevealTx = Extract<AccountTx, { type: 'cross_pull_reveal' }>;
...
/**
 * Pay-as-you-go settlement of one committed fill level.
 *
 * `cross_pull_progress`/fill ACKs advance only binding bookkeeping; until a
 * terminal close the dispute ProofBody carried `claimedRatio = 0` and the whole
 * pull rode on argument evidence. This reveal moves the already-committed exact
 * increment into offdelta NOW and restates `pull.claimedRatio`, so the level
 * survives any later account collapse without any argument evidence at all
 * (DeltaTransformer.applyPull pays only above `claimedRatio`).
 *
 * Baseline quantization: the moved value is the exact committed amount
 * (binding.filledXAmount), while the on-chain baseline is the uint16 ratio's
 * floor claim Q(r) = A*r/65535. With the route's `uint16_ceil` rounding
 * Q(r) >= exact(r), so a disputed later increment Q(r2)-Q(r1) can diverge from
 * exact by at most ceil(A/65535) per pull — the same dust bound the route
 * settlement policy already accepts (`maxSourceDust`/`maxTargetDust`).
 */
export async function handleCrossPullReveal(
  account: AccountState,
  accountTx: CrossPullRevealTx,
  byLeft: boolean,
  currentTimestamp: number,
): Promise<{ success: boolean; events: string[]; error?: string }> {
  ...
  const events: string[] = [];
  const pull = account.pulls?.get(pullId);
  // A reveal can race the terminal close built from the same ladder secrets.
  // The close already settled the exact cumulative amount, so treat the late
  // reveal as idempotent instead of poisoning the whole frame.
  if (!pull) {
    return {
      success: true,
      events: [`🪝 Cross-j pull reveal ignored: ${pullId.slice(0, 8)}... already closed`],
    };
  }
  const binding = pull.crossJurisdiction;
  if (!binding) return { success: false, error: `Cross-j reveal requires pull binding`, events };

  // Same authority as close: the leg's Hub authors settlement economics
  // (source Hub is the source-pull beneficiary; target Hub is the target payer).
  const beneficiaryIsLeft = pull.amount > 0n;
  const authorizedHubIsLeft = binding.leg === 'source' ? beneficiaryIsLeft : !beneficiaryIsLeft;
  if (byLeft !== authorizedHubIsLeft) {
    return { success: false, error: `Only the ${binding.leg} Hub can reveal cross-j pull`, events };
  }
  ...
  try {
    ...
    return {
      success: false,
      error: `Invalid cross-j reveal binary: ${error instanceof Error ? error.message : String(error)}`,
      events,
    };
  }

  // The reveal must land exactly on the bilaterally committed fill level.
  // A lower reveal would under-collateralize the baseline restated below; a
  // higher one would move value no fill ACK ever authorized.
  let committedRatio: number;
  try {
    committedRatio = committedCrossJurisdictionRatio(binding);
  } catch (error) {
    return { success: false, error: error instanceof Error ? error.message : String(error), events };
  }
  if (decodedRatio !== committedRatio) {
    return {
      success: false,
      error: `Cross-j reveal ratio mismatch: binary ${decodedRatio} != committed ${committedRatio}`,
      events,
    };
  }
  const previousRatio = Math.max(0, Math.min(HASHLADDER_MAX_FILL_RATIO, Math.floor(Number(pull.claimedRatio ?? 0) || 0)));
  if (decodedRatio <= previousRatio) {
    // Replays after crash/resend are expected; the level is already settled.
    return {
      success: true,
      events: [`🪝 Cross-j pull reveal replayed: ${pullId.slice(0, 8)}... ratio ${decodedRatio}`],
    };
  }
  ...
  const absAmount = absBigInt(pull.amount);
  const cumulativeClaimed = binding.leg === 'source'
    ? binding.filledSourceAmount
    : binding.filledTargetAmount;
  if (cumulativeClaimed === undefined) {
    return { success: false, error: `Cross-j reveal requires committed fill amounts`, events };
  }
  const previousClaimed = pull.claimedAmount ?? 0n;
  if (cumulativeClaimed > absAmount) {
    return { success: false, error: `Cross-j reveal amount overflow: ${cumulativeClaimed} > ${absAmount}`, events };
  }
  if (cumulativeClaimed <= previousClaimed) {
    return { success: false, error: `Cross-j reveal amount regression: ${cumulativeClaimed} <= ${previousClaimed}`, events };
  }
  const applied = cumulativeClaimed - previousClaimed;

  const delta = ensureDelta(account, pull.tokenId);
  const payerIsLeft = !beneficiaryIsLeft;
  // Only the settled increment leaves the hold; the unfilled remainder stays
  // reserved for future fills (terminal close/expire releases it later).
  const holdError = releaseHold(
    delta,
    payerIsLeft ? 'left' : 'right',
    applied,
  ...
  pull.claimedRatio = decodedRatio;
  pull.claimedAmount = cumulativeClaimed;

  events.push(
    `🪝 Cross-j pull revealed: ${pullId.slice(0, 8)}... ratio ${decodedRatio}/${HASHLADDER_MAX_FILL_RATIO} claimed +${applied} (total ${cumulativeClaimed})`,
  );
  return { success: true, events };
}
