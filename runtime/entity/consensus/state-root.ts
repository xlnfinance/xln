...
export const projectEntityConsensusState = (state: EntityState): Record<string, unknown> => {
  ...
  return {
    ...
    accounts: new Map(Array.from(state.accounts.entries()).map(([counterpartyId, account]) => [
      counterpartyId,
...
export const computeEntityFrameAuthorityRoot = (authority: EntityFrameAuthority): string =>
  ethers.keccak256(ethers.toUtf8Bytes(encodeCanonicalEntityConsensusValue({
    domain: 'xln.entity.frame-authority.v2',
    authority: {
