...
const SCOPES = [
  '/runtime/orderbook/',
  '/runtime/storage/',
  '/runtime/api/runtime-adapter/',
  '/runtime/entity/tx/handlers/account/orderbook/',
  '/runtime/scripts/operations/benchmark/production-swap-load/',
];
...
for (const source of program.getSourceFiles()) {
  ...
  if (!path.includes('/runtime/') || path.includes('/runtime/__tests__/')) continue;
  if (SCOPES.some(scope => path.includes(scope))) visitLegacy(source, source);
  if (path.endsWith('/runtime/orderbook/core.ts')) checkHotMatcher(source);
  if (
    path.endsWith('/runtime/entity/tx/handlers/account/orderbook/same/pass.ts') ||
    path.endsWith('/runtime/entity/tx/handlers/account/orderbook/cross/book.ts')
  ) checkAuthorityPass(source);
}

const storageFiles = program.getSourceFiles().filter(source =>
  source.fileName.replaceAll('\\', '/').includes('/runtime/storage/'));
...
const bookCodec = ts.sys.readFile('runtime/storage/schema/book-graph-codec.ts') ?? '';
if (!bookCodec.includes('projectStorageBookGraphChanges')) {
  violations.push('runtime/storage/schema/book-graph-codec.ts:dirty-path-writer-missing');
}
const storageIndex = ts.sys.readFile('runtime/storage/index.ts') ?? '';
if (!storageIndex.includes('bookGraphWrites')) {
  violations.push('runtime/storage/index.ts:book-graph-batch-missing');
}
