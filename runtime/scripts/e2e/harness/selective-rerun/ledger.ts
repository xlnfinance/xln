  ...
  const table = unresolved
    .map(entry => {
      const rerun = entry.kind === 'scenario'
        ? `bun runtime/scenarios/run.ts ${entry.target} --mode=rpc --single`
        : `bun runtime/scripts/e2e/runners/run-e2e-parallel-isolated.ts --shards=1 --workers-per-shard=1 --pw-files=${entry.target} --strict-browser-health --preserve-artifacts`;
