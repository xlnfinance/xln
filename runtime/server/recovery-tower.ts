...
import { buildTowerAppointmentOwnerMessage } from '../recovery/crypto';
import type { TowerBackupStore } from './tower-backup';
...
export const handleTowerAppointment = async (
  ...
  try {
    ...
    return new Response(serializeTaggedJson({ ok: true, receipt }), { headers });
  } catch (error) {
    return new Response(
...
export const handleTowerRestore = async (
  req: Request,
  headers: HeadersInit,
  store: TowerBackupStore,
): Promise<Response> => {
  try {
    ...
    if (!restored) {
      return new Response(serializeTaggedJson({ ok: false, error: 'TOWER_BUNDLE_NOT_FOUND' }), { status: 404, headers });
    }
    const response: TowerRestoreResponseV1 = { ok: true, receipt: restored.receipt, bundle: restored.bundle };
    return new Response(serializeTaggedJson(response), { headers });
  } catch (error) {
    return new Response(
      serializeTaggedJson({ ok: false, error: error instanceof Error ? error.message : String(error) }),
      { status: 400, headers },
...
export const handleTowerReceipt = async (
  lookupKey: string,
  headers: HeadersInit,
  store: TowerBackupStore,
): Promise<Response> => {
  try {
    ...
    if (!receipt) {
      return new Response(serializeTaggedJson({ ok: false, error: 'TOWER_RECEIPT_NOT_FOUND' }), { status: 404, headers });
    }
    return new Response(serializeTaggedJson({ ok: true, receipt }), { headers });
  } catch (error) {
    return new Response(
...
export const handleRecoveryDiscover = async (
  req: Request,
  headers: HeadersInit,
  store: TowerBackupStore,
): Promise<Response> => {
...
export const handleRecoveryState = async (
  req: Request,
  headers: HeadersInit,
  store: TowerBackupStore,
): Promise<Response> => {
  return handleTowerRestore(req, headers, store);
};

export const handleRecoveryComplaint = async (
  req: Request,
  headers: HeadersInit,
  store: TowerBackupStore,
): Promise<Response> => {
  try {
    ...
    return new Response(serializeTaggedJson({ ok: true }), { headers });
  } catch (error) {
    return new Response(
      serializeTaggedJson({ ok: false, error: error instanceof Error ? error.message : String(error) }),
      { status: 400, headers },
