...
import {
  decodeReliableDeliveryReceipt,
  validateReliableDeliveryIdentity,
} from '../../../runtime/reliable/reliable-receipt-schema';
import {
  requireArray,
...
export const validateReliableReceipt = (value: unknown, code: string): void => {
  decodeReliableDeliveryReceipt(value, code);
};
...
const validatePendingIngress = (value: unknown, code: string): void => {
  ...
  validateReliableDeliveryIdentity(pending['identity'], `${code}_IDENTITY`);
