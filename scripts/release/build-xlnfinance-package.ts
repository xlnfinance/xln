...
const assertPortableServerBundle = (): void => {
  ...
  if (!statSync(launcherClient).isFile()) throw new Error(`XLNFINANCE_LAUNCHER_CLIENT_MISSING:${launcherClient}`);
};
...
const pairPackedRuntime = async (state: string): Promise<RemoteRuntimeAdapter> => {
  ...
  while (Date.now() < deadline) {
    let adapter: RemoteRuntimeAdapter | null = null;
    try {
...
const smokePackedPackage = async (version: string): Promise<void> => {
  ...
  const state = join(stateRoot, 'testnet');
  ...
  try {
    ...
    if (!browserRevealDenied) throw new Error('XLNFINANCE_PACKAGE_SMOKE_BROWSER_RECOVERY_NOT_DENIED');
    const ownerPath = join(state, 'brainvault-owner.json');
    if ((statSync(ownerPath).mode & 0o777) !== 0o600) {
      throw new Error('XLNFINANCE_PACKAGE_SMOKE_BRAINVAULT_OWNER_MODE_INVALID');
    }
    ...
    command([executable, 'stop']);
    command([executable, 'daemon', '--dev']);
    command([executable, 'status']);
    const devState = join(stateRoot, 'dev');
    const devConfig = JSON.parse(readFileSync(join(devState, 'jurisdictions.json'), 'utf8')) as {
      jurisdictions?: { arrakis?: { rpc?: string } };
    };
    if (devConfig.jurisdictions?.arrakis?.rpc !== 'http://127.0.0.1:20740') {
      throw new Error('XLNFINANCE_PACKAGE_SMOKE_DEV_CONFIG_INVALID');
    }
    const devAdapter = await pairPackedRuntime(devState);
    await devAdapter.read('head');
    devAdapter.disconnect();
    command([executable, 'stop']);
    console.log(
      `xlnfinance ${version}: BrainVault ${brainVault.ethereumAddress} (${brainVault.derivationTimeMs}ms, ${brainVault.workers}w), `
      + `admin write ${before}->${committed}, restart restored ${restored}, dev isolated`,
    );
  } finally {
    if (executable) {
