...
import { ConsensusConfig, JurisdictionConfig } from './types.js';
...
export const getContractAddress = async (port: string): Promise<string> => {
  let config: any;
...
export const connectToEthereum = async (rpcUrl: string = 'http://localhost:8545', contractAddress?: string) => {
...
export const assignNameOnChain = async (name: string, entityNumber: number): Promise<{txHash: string}> => {
  try {
    const { entityProvider } = await connectToEthereum();
...
export const getEntityInfoFromChain = async (entityId: string): Promise<{exists: boolean, entityNumber?: number, name?: string}> => {
  try {
    const { entityProvider } = await connectToEthereum();
...
export const getNextEntityNumber = async (port: string = '8545'): Promise<number> => {
  try {
