...
export const applyEntityInput = async (
  ...
  logger.consensus(
    ...
    { entityId: entityInput.entityId, signerId: entityInput.signerId }
  );
  logger.consensus(
    ...
    { entityId: entityInput.entityId, signerId: entityInput.signerId, mempoolSize: entityReplica.mempool.length }
  );
  logger.consensus(
    `INPUT-DETAILS: txs=${entityInput.entityTxs?.length || 0}, precommits=${entityInput.precommits?.size || 0}, frame=${frameHash}`,
    { entityId: entityInput.entityId, signerId: entityInput.signerId, txCount: entityInput.entityTxs?.length || 0, precommitCount: entityInput.precommits?.size || 0 }
  );
  ...
  if (entityInput.entityTxs?.length) {
    ...
    if (entityReplica.signerId === 'alice') {
      ...
      logger.consensus(
        `ALICE-RECEIVES: Transaction types: ${entityInput.entityTxs.map(tx => tx.type).join(', ')}`,
        { entityId: entityInput.entityId, signerId: entityReplica.signerId, transactionTypes: entityInput.entityTxs.map(tx => tx.type) }
      );
      logger.consensus(
        `ALICE-RECEIVES: Alice isProposer=${entityReplica.isProposer}, current mempool=${entityReplica.mempool.length}`,
        { entityId: entityInput.entityId, signerId: entityReplica.signerId, isProposer: entityReplica.isProposer, mempoolSize: entityReplica.mempool.length }
      );
    }
    ...
    if (DEBUG)
      logger.debug(
        ...
        { entityId: entityInput.entityId, signerId: entityInput.signerId, addedTxs: entityInput.entityTxs.length, totalMempool: entityReplica.mempool.length }
      );
    ...
    if (DEBUG) logger.warn(`CORNER CASE: Empty transaction array received - no mempool changes`, { entityId: entityInput.entityId, signerId: entityInput.signerId });
  }
  ...
  if (!entityReplica.isProposer && entityReplica.mempool.length > 0) {
    if (DEBUG) logger.debug(`Non-proposer sending ${entityReplica.mempool.length} txs to proposer`, { entityId: entityInput.entityId, signerId: entityInput.signerId, mempoolSize: entityReplica.mempool.length });
    ...
    logger.consensus(
      `BOB-TO-ALICE: Transaction types: ${entityReplica.mempool.map(tx => tx.type).join(', ')}`,
      { entityId: entityInput.entityId, signerId: entityInput.signerId, transactionTypes: entityReplica.mempool.map(tx => tx.type) }
    );
  ...
  if (entityInput.precommits?.size && entityInput.proposedFrame && !entityReplica.proposal) {
    ...
    if (totalPower >= entityReplica.state.config.threshold) {
      ...
      if (DEBUG) logger.debug(`Received commit notification with ${entityInput.precommits.size} signatures`, { entityId: entityInput.entityId, signerId: entityInput.signerId, precommitCount: entityInput.precommits.size });
      ...
      if (DEBUG)
        logger.debug(
          ...
          { entityId: entityInput.entityId, signerId: entityInput.signerId, messageCount: entityReplica.state.messages.length, height: entityReplica.state.height }
        );
  ...
  if (
    ...
    if (DEBUG) logger.debug(`Validator locked to frame ${entityInput.proposedFrame.hash.slice(0, 10)}...`, { entityId: entityInput.entityId, signerId: entityReplica.signerId, frameHash: entityInput.proposedFrame.hash.slice(0, 10) });
    ...
    if (config.mode === 'gossip-based') {
      ...
      config.validators.forEach(validatorId => {
        if (validatorId !== entityReplica.signerId) {
          logger.consensus(
            ...
            { entityId: entityInput.entityId, signerId: entityReplica.signerId, targetValidator: validatorId, proposalHash: frameHash }
          );
      ...
      if (DEBUG) logger.debug(`Signed proposal, gossiping precommit to ${config.validators.filter(v => v !== entityReplica.signerId).length} validators`, { entityId: entityInput.entityId, signerId: entityReplica.signerId, validatorCount: config.validators.filter(v => v !== entityReplica.signerId).length });
    } else {
      ...
      logger.consensus(
        ...
        { entityId: entityInput.entityId, signerId: entityReplica.signerId, proposerId, proposalHash: frameHash }
      );
      logger.consensus(
      ...
      if (DEBUG) logger.debug(`Signed proposal, sending precommit to ${proposerId}`, { entityId: entityInput.entityId, signerId: entityReplica.signerId, proposerId });
    }
  }
  ...
  if (entityInput.precommits?.size && entityReplica.proposal) {
    ...
    if (DEBUG)
      logger.debug(
        ...
        { entityId: entityInput.entityId, signerId: entityInput.signerId, newSignatures: entityInput.precommits.size, totalSignatures: entityReplica.proposal.signatures.size }
      );
    ...
    if (DEBUG) {
      ...
      logger.info(
        ...
        { entityId: entityInput.entityId, signerId: entityInput.signerId, totalPower: Number(totalPower), totalShares: Number(totalShares), percentage }
      );
    ...
    if (totalPower >= entityReplica.state.config.threshold) {
      ...
      if (DEBUG) logger.debug(`Threshold reached! Committing frame, height: ${entityReplica.state.height}`, { entityId: entityInput.entityId, signerId: entityInput.signerId, height: entityReplica.state.height });
      ...
      if (entityReplica.state.config.mode === 'proposer-based') {
        ...
        logger.consensus(
          ...
          { entityId: entityInput.entityId, signerId: entityReplica.signerId, proposalHash: committedProposalHash }
        );
        ...
        entityReplica.state.config.validators.forEach(validatorId => {
          if (validatorId !== entityReplica.signerId) {
            ...
            logger.consensus(
              ...
              { entityId: entityInput.entityId, signerId: entityReplica.signerId, targetValidator: validatorId, proposalHash: committedProposalHash, precommitCount: sortedSignatures.size, precommitSigners }
            );
        ...
        if (DEBUG) logger.debug(`Sending commit notifications to ${notifiedCount} validators (excluding self)`, { entityId: entityInput.entityId, signerId: entityReplica.signerId, notifiedCount });
      } else {
        logger.consensus(
          ...
          { entityId: entityInput.entityId, signerId: entityReplica.signerId }
        );
        if (DEBUG)
  ...
  if (entityReplica.mempool.length > 0) {
    logger.consensus(
      `  mempoolTypes: ${entityReplica.mempool.map(tx => tx.type).join(', ')}`,
      { entityId: entityReplica.entityId, signerId: entityReplica.signerId, mempoolTypes: entityReplica.mempool.map(tx => tx.type) }
    );
  }
  ...
  if (entityReplica.isProposer && entityReplica.mempool.length > 0 && !entityReplica.proposal) {
    ...
    logger.consensus(
      ...
      { entityId: entityReplica.entityId, signerId: entityReplica.signerId, mempoolSize: entityReplica.mempool.length, isProposer: entityReplica.isProposer, hasProposal: !!entityReplica.proposal }
    );
    logger.consensus(
      `ALICE-PROPOSES: Mempool transaction types: ${entityReplica.mempool.map(tx => tx.type).join(', ')}`,
      { entityId: entityReplica.entityId, signerId: entityReplica.signerId, transactionTypes: entityReplica.mempool.map(tx => tx.type) }
    );
    ...
    if (isSingleSigner) {
      ...
      if (DEBUG)
        logger.debug(
          ...
          { entityId: entityReplica.entityId, signerId: entityReplica.signerId, height: entityReplica.state.height }
        );
    ...
    if (DEBUG)
      logger.debug(
        ...
        { entityId: entityReplica.entityId, signerId: entityReplica.signerId, frameHash: entityReplica.proposal.hash, txCount: entityReplica.proposal.txs.length }
      );
    ...
    if (DEBUG) logger.warn(`CORNER CASE: Proposer with empty mempool - no auto-propose`, { entityId: entityReplica.entityId, signerId: entityReplica.signerId });
  } else if (!entityReplica.isProposer && entityReplica.mempool.length > 0) {
    if (DEBUG) logger.debug(`Non-proposer sending ${entityReplica.mempool.length} txs to proposer`, { entityId: entityReplica.entityId, signerId: entityReplica.signerId, mempoolSize: entityReplica.mempool.length });
    ...
    if (DEBUG) logger.warn(`CORNER CASE: Proposer already has pending proposal - no new auto-propose`, { entityId: entityReplica.entityId, signerId: entityReplica.signerId });
  }
  ...
  logger.consensus(
    ...
    { entityId: entityReplica.entityId, signerId: entityReplica.signerId, outputCount: entityOutbox.length }
  );
  logger.consensus(
  ...
  entityOutbox.forEach((output, index) => {
    ...
    logger.consensus(
      ...
      { entityId: output.entityId, signerId: output.signerId, txCount: output.entityTxs?.length || 0, precommitCount: output.precommits?.size || 0, frameHash: outputFrameHash }
    );
    ...
    if (merged.has(key)) {
      ...
      if (input.precommits) {
        ...
        logger.debug(
          ...
          { entityId: input.entityId, signerId: input.signerId, newPrecommits: input.precommits.size, existingPrecommits: existingPrecommits.size }
        );
      ...
      logger.debug(
        ...
        { entityId: input.entityId, signerId: input.signerId, txCount: input.entityTxs?.length || 0, precommitCount: input.precommits?.size || 0, hasFrame: !!input.proposedFrame }
      );
    } else {
