...
export function idGet(id: number): number {
  id = id >>> 0;
  return (id < DENSE_LIMIT) ? dense[id] : sparse.get(id);
}
export function idSet(id: number, idx: number): void {
  id = id >>> 0;
  if (id < DENSE_LIMIT) dense[id] = idx | 0;
  else sparse.set(id, idx | 0);
}
export function idDel(id: number): void {
  id = id >>> 0;
  if (id < DENSE_LIMIT) dense[id] = -1;
  else sparse.del(id);
}
export function idHas(id: number): boolean { return idGet(id) !== -1; }
export function idClear(): void { dense.fill(-1); sparse.clear(); }