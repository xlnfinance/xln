...
// ── Ядро: заявки ──────────────────────────────────────────────────────────────
function newOrder(owner:number, clientId:number, side:Side, priceTicks:number, qtyLots:number, tifGTCorIOC:0|1=0, postOnly=false) {
  if (qtyLots <= 0) { emit({kind:'REJECT'}); return; }
  const levelIdx = ((priceTicks - PRICE_MIN) / TICK) | 0;
  if (levelIdx < 0 || levelIdx >= LEVELS) { emit({kind:'REJECT'}); return; }
  ...
  if (side === 0) { // BUY
    if (postOnly && bestAskIdx !== EMPTY && bestAskIdx <= levelIdx) { emit({kind:'REJECT'}); return; }
    while (remaining > 0 && bestAskIdx !== EMPTY && bestAskIdx <= levelIdx) {
      remaining = fillAgainstLevel(bestAskIdx, 0, remaining, (px, q)=>{ filledLots+=q; filledNotionalTicks += px*q; });
      if (bestAskIdx !== EMPTY && levelHead[bestAskIdx] === EMPTY) bestAskIdx = findNextNonEmptyFrom(bestAskIdx + 1);
    }
  } else {        // SELL
    if (postOnly && bestBidIdx !== EMPTY && bestBidIdx >= levelIdx) { emit({kind:'REJECT'}); return; }
    while (remaining > 0 && bestBidIdx !== EMPTY && bestBidIdx >= levelIdx) {
      remaining = fillAgainstLevel(bestBidIdx, 1, remaining, (px, q)=>{ filledLots+=q; filledNotionalTicks += px*q; });
      if (bestBidIdx !== EMPTY && levelHead[bestBidIdx] === EMPTY) bestBidIdx = findPrevNonEmptyFrom(bestBidIdx - 1);
    }
  }

  if (remaining > 0) {
    if (tifGTCorIOC === 1) { // IOC
...
function fillAgainstLevel(levelIdx:number, takerSide:Side, remaining:number,
  acc:(priceTicks:number, qty:number)=>void): number {
  while (remaining > 0) {
    const head = levelHead[levelIdx];
    ...
    if (newMakerQty === 0) { popHead(levelIdx); freeOrder(head); emit({kind:'FILLED'}); }
    else { emit({kind:'REDUCED'}); break; } // частично — остаётся в голове
  }
  return remaining;
}

function cancel(_owner:number, idx:number) {
  if (idx < 0 || idx >= MAX_ORDERS || orderActive[idx] === 0) { emit({kind:'REJECT'}); return; }
  // lazy-cancel
  orderActive[idx] = 0;
  emit({kind:'CANCELED'});
}
