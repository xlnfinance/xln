...
function ctz32(x: number): number { return (Math.clz32((x & -x) >>> 0) ^ 31); }
function findNextNonEmptyFrom(i: number): number { // для ask
  if (i < 0) i = 0;
  for (; i < LEVELS; ) {
...
function findPrevNonEmptyFrom(i: number): number { // для bid
...
const P = 0x1_0000_01n;          // небольшой прост. модуль для mix

function emitACK(){ if (!EMIT_EVENTS) return; evAck++; }
function emitREJECT(){ if (!EMIT_EVENTS) return; evReject++; }
function emitTRADE(pxTicks:number, qty:number){
  if (!EMIT_EVENTS) return;
  evTrade++;
  if (COUNT_EVENTS_ONLY) return; // ускорение: без BigInt
...
function emitFILLED(){ if (!EMIT_EVENTS) return; evFilled++; }
function emitREDUCED(){ if (!EMIT_EVENTS) return; evReduced++; }
function emitCANCELED(){ if (!EMIT_EVENTS) return; evCanceled++; }

// -------------------- Ядро (NEW / CANCEL / REDUCE) ---------------------------
function newOrder(owner:number, _clientId:number, side:Side, priceTicks:number, qtyLots:number, tif:0|1=0, postOnly=false) {
...
function fillAgainstLevel(levelIdx:number, remaining:number, acc:(pxTicks:number, qty:number)=>void): number {
  while (remaining > 0) {
    const head = levelHead[levelIdx];
    if (head === EMPTY) return remaining;
    if (orderActive[head] === 0) { popHead(levelIdx); freeOrder(head); continue; }
    ...
    if (newMakerQty === 0) { popHead(levelIdx); freeOrder(head); emitFILLED(); }
    else { emitREDUCED(); break; } // частично — остаётся в голове
  }
  return remaining;
}

// Отмена head уровня (детерминированно выбираем уровень)
function cancelHeadAtLevel(levelIdx: number) {
  const head = levelHead[levelIdx];
  if (head === EMPTY) { emitREJECT(); return; }
...
function reduceHeadAtLevel(levelIdx:number, lots:number) {
  const head = levelHead[levelIdx];
  if (head === EMPTY) { emitREJECT(); return; }
...
// -------------------- Предгенерация команд -----------------------------------
type CmdKind = 0|1|2; // 0=ADD, 1=CANCEL, 2=REDUCE
...
function chooseNonEmptyLevelForCancelOrReduce(preferBid:boolean): number {
...
function pregen() {
  for (let i=0;i<OPS;i++){
    ...
    if (r < ADD_PCT) {
      ...
      cmdLvl[i] = li;
      cmdQty[i] = ((rndU32()%5|0)+1) as number;
      ...
      const lvl = chooseNonEmptyLevelForCancelOrReduce(preferBid);
      cmdLvl[i] = (lvl === EMPTY) ? -1 : lvl;
      cmdQty[i] = ((rndU32()%3|0)+1) as number; // уменьшить на 1..3 лота
    }
  }
}
...
function runBench() {
  ...
  for (; i<limit; i+=UNROLL){
    for (let k=0;k<UNROLL;k++){
      ...
      if (kind === 0){
        ...
        const li = cmdLvl[ii];
        if (li >= 0) cancelHeadAtLevel(li);
        else emitREJECT();
        cancels++;
      } else {
        const li = cmdLvl[ii];
        if (li >= 0) reduceHeadAtLevel(li, cmdQty[ii] || 1);
        else emitREJECT();
        reduces++;
      }
    }
  ...
  for (; i<OPS; i++){
    ...
    if (kind === 0){
      const li = cmdLvl[i];
      const px = PRICE_MIN + li*TICK;
      newOrder(nextOwner, nextOwner, cmdSide[i] as Side, px, cmdQty[i], 0, false);
      nextOwner++; adds++;
    } else if (kind === 1){
      const li = cmdLvl[i];
      if (li >= 0) cancelHeadAtLevel(li); else emitREJECT();
      cancels++;
    } else {
      const li = cmdLvl[i];
      if (li >= 0) reduceHeadAtLevel(li, cmdQty[i] || 1); else emitREJECT();
      reduces++;
    }
  }
...
function saveSnapshots(db: Record<string,string>) {
