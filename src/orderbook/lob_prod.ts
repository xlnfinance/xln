...
const getNum = (k:string, d:number) => {
  const a = argv.find(x => x.startsWith(`--${k}=`)); return a ? Number(a.split('=')[1]) : d;
};
const getBool = (k:string, d:boolean=false) => argv.some(x => x===`--${k}` || x===`--${k}=1` || x===`--${k}=true`) ? true : d;
...
type CmdKind = 0|1|2; // 0 ADD, 1 CANCEL, 2 REPLACE

// Command (binary WAL layout fixed-width)
// ADD: kind=0, owner,u64 orderId, side,u8, tif,u8, flags,u8, price i64, qty u32, replace fields zero
// CANCEL: kind=1, owner, orderId
// REPLACE: kind=2, owner, orderId, newPrice i64 (or INT64_MIN if none), newQtyDeltaI32 (negative to reduce, positive to increase)
interface AddCmd { kind:0; owner:number; orderId:number; side:Side; tif:TIF; postOnly:boolean; reduceOnly:boolean; priceTicks:number; qtyLots:number; }
interface CancelCmd { kind:1; owner:number; orderId:number; }
interface ReplaceCmd { kind:2; owner:number; orderId:number; newPriceTicks:number|null; qtyDeltaLots:number; }
type Cmd = AddCmd|CancelCmd|ReplaceCmd;

// Events (egress minimal, idempotent with seqOut)
type Ev =
  | { k:'ACK', id:number }
  | { k:'REJECT', id:number, r:string }
  | { k:'TRADE', mk:number, tk:number, px:number, q:number, ms:number, ts:number }
  | { k:'FILLED', id:number }
  | { k:'REDUCED', id:number, d:number, rem:number }
  | { k:'CANCELED', id:number, r:string };
...
// ---------- Bit helpers ----------
const setBit=(i:number)=>{ const w=(i/BITWORD|0),b=i&31; bitmap[w]|=(1<<b)>>>0; };
const clearBit=(i:number)=>{ const w=(i/BITWORD|0),b=i&31; bitmap[w]&=(~(1<<b))>>>0; };
...
function enqueueTail(levelIdx:number, idx:number){
  const tail = levelTail[levelIdx];
  if (tail===EMPTY){
    levelHead[levelIdx]=idx; levelTail[levelIdx]=idx;
    ...
    orderNext[tail]=idx; orderPrev[idx]=tail; orderNext[idx]=EMPTY; levelTail[levelIdx]=idx;
  }
}
function removeFromLevel(levelIdx:number, idx:number){
  const p=orderPrev[idx], n=orderNext[idx];
  if (p!==EMPTY) orderNext[p]=n; else levelHead[levelIdx]=n;
  if (n!==EMPTY) orderPrev[n]=p; else levelTail[levelIdx]=p;
...
function freeOrder(i:number){ orderActive[i]=0; orderNext[i]=freeTop; freeTop=i; }
...
const outbox: Ev[] = []; // not stored in bench to avoid GC; in prod use typed outbox ring.
function emitACK(){ evAck++; /*outbox.push({k:'ACK', id})*/ }
function emitREJECT(){ evReject++; }
function emitTRADE(px:number, qty:number){
  evTrade++;
...
function stp(ownerMaker:number, ownerTaker:number): 0|1|2 {
  if (ownerMaker!==ownerTaker) return 0; // off
  if (STP_POLICY===1) return 1; // cancel taker
  if (STP_POLICY===2) return 2; // decrement maker
  return 0;
}

// ---------- Matching core ----------
function newOrder(owner:number, orderId:number, side:Side, priceTicks:number, qtyLots:number, tif:TIF=0, postOnly=false, reduceOnly=false){
  if (qtyLots<=0){ emitREJECT(); return; }
  const levelIdx=((priceTicks-PRICE_MIN)/TICK)|0;
  if (levelIdx<0 || levelIdx>=LEVELS){ emitREJECT(); return; }

  // Reduce-only: taker cannot increase position; for spot demo, treat as normal (hook for perps).
  let remaining = qtyLots>>>0;

  if (side===0){ // BUY
    if (postOnly && bestAskIdx!==EMPTY && bestAskIdx<=levelIdx){ emitREJECT(); return; }
    ...
    if (postOnly && bestBidIdx!==EMPTY && bestBidIdx>=levelIdx){ emitREJECT(); return; }
  ...
  if (remaining>0){
    if (tif===1){ // IOC: do not rest
      if (qtyLots!==remaining) emitFILLED(); else emitREJECT();
      return;
    }
    // rest as maker
...
function fillAgainstLevel(levelIdx:number, remaining:number, takerOwner:number, takerSide:Side): number {
  while (remaining>0){
    const head = levelHead[levelIdx];
    if (head===EMPTY) return remaining;
    if (!orderActive[head]){ removeFromLevel(levelIdx, head); freeOrder(head); continue; }

    // STP
    const s = stp(orderOwner[head], takerOwner);
    if (s===1){ // cancel taker
      emitREJECT(); return remaining; // taker aborted
    }
    ...
    if (s===2){ // decrement maker (reduce maker so it doesn't cross with self)
      if (makerQty>0){
        ...
        emitREDUCED();
        if (orderQtyLots[head]===0){ removeFromLevel(levelIdx, head); freeOrder(head); emitFILLED(); }
...
function cancel(owner:number, orderId:number){
  const idx = (orderId<MAX_ORDERS)? orderId2Idx[orderId] : EMPTY;
  if (idx===EMPTY || !orderActive[idx]){ emitREJECT(); return; }
...
function replace(owner:number, orderId:number, newPriceTicks:number|null, qtyDeltaLots:number){
  const idx = (orderId<MAX_ORDERS)? orderId2Idx[orderId] : EMPTY;
  if (idx===EMPTY || !orderActive[idx]){ emitREJECT(); return; }
  ...
  if (wantQty<=0){ // becomes cancel
    const lvl=orderPriceIdx[idx];
    removeFromLevel(lvl, idx); freeOrder(idx); orderId2Idx[orderId]=EMPTY; emitCANCELED(); return;
  }
  ...
  if (newPriceTicks!==null){
    ...
    if (li<0 || li>=LEVELS){ emitREJECT(); return; }
    if (li!==targetLevelIdx){ changedPrice=true; targetLevelIdx=li; }
  }
  if (qtyDeltaLots>0) changedUp=true;
...
let walFd:number|undefined;
let walBuf:Buffer = Buffer.allocUnsafe(0);
let walTimer:NodeJS.Timeout|undefined;

function walOpen(){
  if (!ENABLE_WAL) return;
  walFd = fs.openSync(WAL_PATH, 'a');
  walTimer = setInterval(()=>walFlush(), WAL_BATCH_MS);
}
function walAppend(cmd:Cmd){
  if (!ENABLE_WAL || walFd===undefined) return;
  ...
  let o=0;
  buf.writeUInt8(cmd.kind, o); o+=1;
  buf.writeUInt32LE((cmd as any).owner>>>0, o); o+=4;
  buf.writeUInt32LE((cmd as any).orderId>>>0, o); o+=4;
  if (cmd.kind===0){
    const c=cmd as AddCmd;
    buf.writeUInt8(c.side, o); o+=1;
    buf.writeUInt8(c.tif, o); o+=1;
    buf.writeUInt8((c.postOnly?1:0) | (c.reduceOnly?2:0), o); o+=1;
    buf.writeBigInt64LE(BigInt(c.priceTicks), o); o+=8;
    buf.writeInt32LE(c.qtyLots|0, o); o+=4;
    buf.writeInt32LE(0, o); o+=4;
  } else if (cmd.kind===1){
    buf.writeUInt8(0,o); o+=1; buf.writeUInt8(0,o); o+=1; buf.writeUInt8(0,o); o+=1;
    buf.writeBigInt64LE(0n, o); o+=8;
    buf.writeInt32LE(0, o); o+=4;
    buf.writeInt32LE(0, o); o+=4;
  } else {
    const c=cmd as ReplaceCmd;
    buf.writeUInt8(0,o); o+=1; buf.writeUInt8(0,o); o+=1; buf.writeUInt8(0,o); o+=1;
    buf.writeBigInt64LE(c.newPriceTicks===null ? -0x8000000000000000n : BigInt(c.newPriceTicks), o); o+=8;
    buf.writeInt32LE(0, o); o+=4;
    buf.writeInt32LE(c.qtyDeltaLots|0, o); o+=4;
  }
  walBuf = Buffer.concat([walBuf, buf]);
}
function walFlush(){
  if (!ENABLE_WAL || walFd===undefined) return;
  if (walBuf.length===0) return;
...
function snapshotWrite(){
  if (!ENABLE_SNAP) return;
  const fd = fs.openSync(SNAP_PATH, 'w');
  // write core arrays
  const writeArr=(a:ArrayBufferLike)=>fs.writeSync(fd, Buffer.from(a as ArrayBuffer));
  ...
  const tl = Buffer.allocUnsafe(4); tl.writeUInt32LE(trailer.length,0);
  fs.writeSync(fd, tl); fs.writeSync(fd, trailer);
  fs.closeSync(fd);
}
...
function computeRestingSummary(){
  ...
  for (let lvl=0; lvl<LEVELS; lvl++){
    ...
    while (cur!==EMPTY){
      if (orderActive[cur]){ restingCount++; restingLots+=BigInt(orderQtyLots[cur]); }
      cur = orderNext[cur];
    }
  }
...
function snapDbLoad():Record<string,string>{
  try{
    if (fs.existsSync(SNAP_DB)){
      const txt=fs.readFileSync(SNAP_DB,'utf8'); return txt? JSON.parse(txt):{};
    }
  }catch{}
  return {};
}
function snapDbSave(db:Record<string,string>){
  fs.writeFileSync(SNAP_DB, JSON.stringify(db,null,2), 'utf8');
}

// ---------- Command application (prod API) ----------
function applyCommand(c:Cmd){
  if (c.kind===0){
    if (ENABLE_WAL) walAppend(c);
...
function chooseCancelLevel():number {
  if (bestBidIdx===EMPTY && bestAskIdx===EMPTY) return EMPTY;
...
function pregen(){
  for (let i=0;i<OPS;i++){
    ...
    if (r<ADD_PCT){
      ...
      cmdLvl[i]=li; cmdPrice[i]=PRICE_MIN + li*TICK; cmdQty[i]=((rndU32()%5|0)+1) as number;
    } else if (r<ADD_PCT+CANCEL_PCT){
      cmdKind[i]=1;
      const lvl=chooseCancelLevel(); cmdLvl[i]=(lvl===EMPTY)?-1:lvl;
    } else {
...
function runBench(){
  ...
  for (; i<limit; i+=UNROLL){
    for (let k=0;k<UNROLL;k++){
      const ii=i+k; const kind=cmdKind[ii];
      if (kind===0){
        const side=cmdSide[ii] as Side;
        const px=cmdPrice[ii]; const qty=cmdQty[ii];
      ...
      if (ENABLE_SNAP && ++snapshotCounter===SNAP_EVERY){ snapshotCounter=0; snapshotWrite(); }
    }
  }
  for (; i<OPS; i++){
    ...
    if (kind===0){
      const id=nextOwner;
      applyCommand({kind:0, owner:nextOwner, orderId:id, side:cmdSide[i] as Side, tif:0, postOnly:false, reduceOnly:false, priceTicks:cmdPrice[i], qtyLots:cmdQty[i]});
      nextOwner++; adds++;
    } else if (kind===1){
      const lvl=cmdLvl[i];
      if (lvl>=0){
        const head=levelHead[lvl];
        if (head!==EMPTY){ const fakeId=orderOwner[head]; applyCommand({kind:1, owner:orderOwner[head], orderId:fakeId}); } else emitREJECT();
      } else emitREJECT();
      cancels++;
    } else {
      const lvl=cmdLvl[i];
      if (lvl>=0){
        const head=levelHead[lvl];
        if (head!==EMPTY){ const fakeId=orderOwner[head]; applyCommand({kind:2, owner:orderOwner[head], orderId:fakeId, newPriceTicks:null, qtyDeltaLots:cmdDelta[i]}); } else emitREJECT();
      } else emitREJECT();
      replaces++;
    }
    if (ENABLE_SNAP && ++snapshotCounter===SNAP_EVERY){ snapshotCounter=0; snapshotWrite(); }
