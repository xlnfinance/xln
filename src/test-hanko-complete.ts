...
import { createHash, isBrowser, randomBytes } from './utils.js';
import {
  buildRealHanko,
  packRealSignatures,
...
const testEdgeCases = async (): Promise<void> => {
  ...
  console.log(
    `   Recovered: ${failedRecovered.yesEntities.length} yes, ${failedRecovered.noEntities.length} placeholders`,
  );
};
...
if (!isBrowser && typeof process !== 'undefined' && import.meta.url === `file://${process.argv[1]}`) {
  runAllTests();
}
