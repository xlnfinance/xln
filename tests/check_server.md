...
Normal e2e runs do not require a pre-running dev server. Use `bun run test:e2e:fast` or `bun runtime/scripts/e2e/runners/run-e2e-parallel-isolated.ts`; the isolated runner starts its own Anvil, runtime API, Vite frontend, and DB root per shard.
