...
describe('frontend capability inventory', () => {
  ...
  test('records the implemented site capability without claiming production cutover', () => {
    ...
    expect(CAPABILITIES.find(({ id }) => id === 'wallet-shell-and-identity')?.behavior)
      .toContain('deterministic browser BrainVault worker sizing and resilience decisions');
    expect(CAPABILITIES.find(({ id }) => id === 'wallet-shell-and-identity')?.behavior)
    ...
    expect(CAPABILITIES.find(({ id }) => id === 'wallet-shell-and-identity')?.behavior)
      .toContain('deterministic node BrainVault access, progress, result, and timing validation');
    expect(CAPABILITIES.find(({ id }) => id === 'wallet-shell-and-identity')?.behavior)
      .toContain('generation-safe node mnemonic reveal coordination');
    expect(CAPABILITIES.find(({ id }) => id === 'wallet-shell-and-identity')?.behavior)
    ...
    expect(CAPABILITIES.find(({ id }) => id === 'wallet-shell-and-identity')?.behavior)
      .toContain('deterministic Runtime preference decoding and serialization');
    expect(CAPABILITIES.find(({ id }) => id === 'wallet-shell-and-identity')?.behavior)
