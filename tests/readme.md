...
| --- | --- | --- |
| L1 unit/component | Pure or near-pure core/frontend/native checks with no browser stack. Use for orderbook, serialization, storage projections, recovery builders, guards. | `bun run test:unit` |
...
| L4 contracts | Solidity jurisdiction contracts and core/contract proof compatibility. | `bun run test:contracts:full` |
...
```bash
bun core/scenarios/run.ts dispute-lifecycle
bun core/scenarios/run.ts --set=smoke --workers=4
```
...
- Put runtime behavior without browser clicks in `core/scenarios/`.
- Put isolated runtime units in `core/__tests__/`.
...
- Do not add silent skips. `core/__tests__/testing/infrastructure/test-skip-discipline.test.ts` enforces this.
...
- `bun run test:e2e:fast` runs the canonical isolated browser flow set from `core/scripts/e2e/runners/run-e2e-fast.ts`.
- `bun run gate:quick` runs source checks, selected core/native tests, soundcheck, and whitespace checks.
