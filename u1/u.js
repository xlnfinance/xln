...
/*
# Compare visually the text below with all other sources
# Make sure everything is exactly the same!
verify=5708c28ed70f5aeb31081a46b1ff4b62f772a424563ab73c1132ca08a38ca4e7
script=$(curl http://get.pow.cx)
if echo script | shasum -a 256 | grep $verify; then
  echo "Installing..."
  # echo script | sh
fi
*/
...
initDashboard=a=>{
  ...
  var server = http.createServer(function(req, res) {
    ...
    assert(req.headers.host == '127.0.0.1:'+wenet_port, 'DNS rebinding attack')
  ...
  me.internal_wss.on('connection', function(ws,req) {
    ...
    assert(req.headers.origin == 'http://127.0.0.1:'+wenet_port, 'DNS rebinding attack')
...
main = async (username, login)=>{
  ...
  if(login){
    ...
    // am I coordinator?
    if(me.coordinators.find(f=>Buffer.compare(f.pubkey, me.id.publicKey) )){
      me.initCoordinator()
